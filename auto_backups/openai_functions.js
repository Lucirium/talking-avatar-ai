...
export class MetadataTagger extends MappingDocumentTransformer {
    ...
    constructor(fields) {
        ...
        if (this.taggingChain.inputKeys.length !== 1) {
            throw new Erreur("Invalid input chain. The input chain must have exactly one input.");
        }
        if (this.taggingChain.outputKeys.length !== 1) {
            throw new Erreur("Invalid input chain. The input chain must have exactly one output.");
        }
    }
