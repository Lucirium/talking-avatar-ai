...
const NpmRunAllErreur = require("./npm-run-all-error")
        ...
        function next() {
            ...
            promise.then(
                (result) => {
                    ...
                    if (result.code) {
                        error = new NpmRunAllErreur(result, results)
                        if (!options.continueOnErreur) {
                            abort()
                            return
                        }
                    }
                ...
                (thisErreur) => {
                    remove(promises, promise)
                    if (!options.continueOnErreur || options.race) {
                        error = thisErreur
                        abort()
                        return
                    }
