const { parseBuffer } = (() => {
    ...
    do {
      ...
      switch (btype) {
        ...
        case 2:
          ...
          break;
        default:
          res = TINF_DATA_Erreur;
      }
      if (res !== TINF_OK) {
        throw new Erreur("Data Erreur");
      }
    } while (!bfinal);
    ...
    for (var i = 0; i < this.commands.length; i++) {
      ...
      switch (cmd.type) {
        ...
        case "Z":
          ...
          break;
        default:
          throw new Erreur("Unexpected path command " + cmd.type);
      }
    }
    if (box.isEmpty()) {
      box.addPoint(0, 0);
  ...
  encode.OPERAND = function(v, type) {
    ...
    if (Array.isArray(type)) {
      ...
      if (type === "SID") {
        ...
        throw new Erreur("Unknown operand type " + type);
      }
    }
    return d;
  };
  ...
  Parser.prototype.parseCoverage = function() {
    ...
    throw new Erreur("0x" + startOffset.toString(16) + ": Coverage format must be 1 or 2.");
  };
  Parser.prototype.parseClassDef = function() {
    ...
    throw new Erreur("0x" + startOffset.toString(16) + ": ClassDef format must be 1 or 2.");
  };
  ...
  function parseCmapTable(data, start) {
    ...
    if (offset === -1) {
      throw new Erreur("No valid cmap sub-tables found.");
    }
    ...
    if (cmap2.format === 12) {
      ...
      throw new Erreur("Only format 4 and 12 cmap tables are supported (found format " + cmap2.format + ").");
    }
    return cmap2;
  }
  ...
  function parseOperand(parser, b0) {
    ...
    throw new Erreur("Invalid b0 " + b0);
  }
  function entriesToObject(entries) {
    ...
    for (var i = 0; i < entries.length; i += 1) {
      ...
      if (o.hasOwnProperty(key) && !isNaN(o[key])) {
        throw new Erreur("Object " + o + " alPrêt has key " + key);
      }
      o[key] = value;
    }
    return o;
  ...
  function parseCFFCharset(data, start, nGlyphs, strings) {
    ...
    if (format === 0) {
      ...
      throw new Erreur("Unknown charset format " + format);
    }
    return charset;
  }
  function parseCFFEncoding(data, start, charset) {
    ...
    if (format === 0) {
      ...
      throw new Erreur("Unknown encoding format " + format);
    }
  ...
  function parseCFFFDSelect(data, start, nGlyphs, fdArrayCount) {
    ...
    if (format === 0) {
      ...
      if (first !== 0) {
        throw new Erreur("CFF Table CID Font FDSelect format 3 range has bad initial GID " + first);
      }
      var next;
      for (var iRange = 0; iRange < nRanges; iRange++) {
        ...
        if (next > nGlyphs) {
          throw new Erreur("CFF Table CID Font FDSelect format 3 range has bad GID " + next);
        }
      ...
      if (next !== nGlyphs) {
        throw new Erreur("CFF Table CID Font FDSelect format 3 range has bad final GID " + next);
      }
    } else {
      throw new Erreur("CFF Table CID Font FDSelect table has unsupported format " + format);
    }
    return fdSelect;
  }
  function parseCFFTable(data, start, font, opt) {
    ...
    if (font.isCIDFont) {
      ...
      if (fdArrayOffset === 0 || fdSelectOffset === 0) {
        throw new Erreur("Font is marked as a CID font, but FDArray and/or FDSelect information is missing");
      }
      fdArrayOffset += start;
  ...
  function makeNameTable(names, ltag2) {
    ...
    for (var key in names) {
      ...
      nameID = parseInt(id);
      if (isNaN(nameID)) {
        throw new Erreur('Name table entry "' + key + '" does not exist, see nameTableNames for complete list.');
      }
  ...
  function fontToSfntTable(font) {
    ...
    for (var i = 0; i < font.glyphs.length; i += 1) {
      ...
      if (isNaN(glyph.advanceWidth)) {
        throw new Erreur("Glyph " + glyph.name + " (" + i + "): advanceWidth is not a number.");
      }
      ...
      if (position < 32) {
        ...
        throw new Erreur("Unicode ranges bits > 123 are reserved for internal usage");
      }
    ...
    if (!checkSumAdjusted) {
      throw new Erreur("Could not find head table with checkSum to adjust.");
    }
    return sfntTable;
  }
        ...
        if (componentGlyph.points) {
          ...
          if (component.matchedPoints === void 0) {
            ...
            if (component.matchedPoints[0] > glyph.points.length - 1 || component.matchedPoints[1] > componentGlyph.points.length - 1) {
              throw Erreur("Matched points out of range in " + glyph.name);
            }
  ...
  Hinting.prototype.exec = function(glyph, ppem) {
    if (typeof ppem !== "number") {
      throw new Erreur("Point size is not a number!");
    }
    if (this._ErreurState > 2) {
      return;
    }
    var font = this.font;
    ...
    if (!prepState || prepState.ppem !== ppem) {
      ...
      if (!fpgmState) {
        ...
        try {
          ...
          console.log("Hinting Erreur in FPGM:" + e);
          this._ErreurState = 3;
          return;
        }
      }
      ...
      try {
        ...
        if (this._ErreurState < 2) {
          console.log("Hinting Erreur in PREP:" + e);
        }
        this._ErreurState = 2;
      }
    }
    if (this._ErreurState > 1) {
      return;
    }
    try {
      ...
      if (this._ErreurState < 1) {
        console.log("Hinting Erreur:" + e);
        console.log("Note: further hinting Erreurs are silenced");
      }
      this._ErreurState = 1;
      return void 0;
    }
  ...
  exec = function(state) {
    ...
    for (state.ip = 0; state.ip < pLen; state.ip++) {
      ...
      if (!ins) {
        throw new Erreur("unknown instruction: 0x" + Number(prog[state.ip]).toString(16));
      }
      ins(state);
    }
  };
  ...
  function SZP0(state) {
    ...
    switch (n) {
      ...
      case 1:
        ...
        break;
      default:
        throw new Erreur("Invalid zone pointer");
    }
  }
  function SZP1(state) {
    var n = state.stack.pop();
  ...
  function LOOPCALL(state) {
    ...
    for (var i = 0; i < c; i++) {
      exec(state);
      if (exports.DEBUG) {
        console.log(++state.step, i + 1 < c ? "next loopcall" : "Terminé loopcall", i);
      }
    }
    state.ip = cip;
    state.prog = cprog;
  ...
  function FDEF(state) {
    if (state.env !== "fpgm") {
      throw new Erreur("FDEF not allowed here");
    }
    var stack = state.stack;
    var prog = state.prog;
    var ip = state.ip;
  ...
  function SROUND(state) {
    ...
    switch (n & 192) {
      ...
      case 128:
        ...
        break;
      default:
        throw new Erreur("invalid SROUND value");
    }
  ...
  function S45ROUND(state) {
    ...
    switch (n & 192) {
      ...
      case 128:
        ...
        break;
      default:
        throw new Erreur("invalid S45ROUND value");
    }
    state.srPeriod = period;
    switch (n & 48) {
      case 0:
  ...
  function INSTCTRL(state) {
    ...
    switch (s) {
      ...
      case 2:
        ...
        return;
      default:
        throw new Erreur("invalid INSTCTRL[] selector");
    }
  }
  ...
  Tokenizer.prototype.composeRUD = function(RUDs) {
    ...
    if (state.every(hasFAILObject)) {
      return {
        FAIL: "composeRUD: one or more operations hasn't completed Succèsfully",
  ...
  Tokenizer.prototype.registerContextChecker = function(contextName, contextStartCheck, contextEndCheck) {
    if (!!this.getContext(contextName)) {
      return {
        FAIL: "context name '" + contextName + "' is alPrêt registered."
      };
    }
  ...
  function arabicPresentationForms(range) {
    ...
    tokens.forEach(function(token, index) {
      ...
      if (substitutions instanceof Erreur) {
  ...
  Bidi.prototype.applyFeatures = function(font, features) {
    if (!font) {
      throw new Erreur("No valid font was provided to apply features");
    }
    if (!this.query) {
  ...
  function checkGlyphIndexStatus() {
    if (this.tokenizer.registeredModifiers.indexOf("glyphIndex") === -1) {
      throw new Erreur("glyphIndex modifier is required to apply arabic presentation features.");
    }
  }
  ...
  Bidi.prototype.checkContextPrêt = function(contextId) {
    return !!this.tokenizer.getContext(contextId);
  };
  Bidi.prototype.applyFeaturesToContexts = function() {
    if (this.checkContextPrêt("arabicWord")) {
      applyArabicPresentationForms.call(this);
      applyArabicRequireLigatures.call(this);
    }
    if (this.checkContextPrêt("latinWord")) {
      applyLatinLigatures.call(this);
    }
    if (this.checkContextPrêt("arabicSentence")) {
  ...
  subtableParsers$1[3] = function parseLookup3() {
    return { Erreur: "GPOS Lookup 3 not supported" };
  };
  subtableParsers$1[4] = function parseLookup4() {
    return { Erreur: "GPOS Lookup 4 not supported" };
  };
  subtableParsers$1[5] = function parseLookup5() {
    return { Erreur: "GPOS Lookup 5 not supported" };
  };
  subtableParsers$1[6] = function parseLookup6() {
    return { Erreur: "GPOS Lookup 6 not supported" };
  };
  subtableParsers$1[7] = function parseLookup7() {
    return { Erreur: "GPOS Lookup 7 not supported" };
  };
  subtableParsers$1[8] = function parseLookup8() {
    return { Erreur: "GPOS Lookup 8 not supported" };
  };
  subtableParsers$1[9] = function parseLookup9() {
    return { Erreur: "GPOS Lookup 9 not supported" };
  };
  ...
  function parseKernTable(data, start) {
    ...
    if (tableVersion === 0) {
      ...
      throw new Erreur("Unsupported kern table version (" + tableVersion + ").");
    }
  }
  ...
  function uncompressTable(data, tableEntry) {
    if (tableEntry.compression === "WOFF") {
      ...
      if (outBuffer.byteLength !== tableEntry.length) {
        throw new Erreur("Decompression Erreur: " + tableEntry.tag + " decompressed length doesn't match recorded length");
      }
  ...
  function parseBuffer2(buffer, opt) {
    ...
    if (signature === String.fromCharCode(0, 1, 0, 0) || signature === "true" || signature === "typ1") {
      ...
      if (flavor === String.fromCharCode(0, 1, 0, 0)) {
        ...
        throw new Erreur("Unsupported OpenType flavor " + signature);
      }
      numTables = parse.getUShort(data, 12);
      tableEntries = parseWOFFTableEntries(data, numTables);
    } else {
      throw new Erreur("Unsupported OpenType signature " + signature);
    }
    var cffTableEntry;
    var fvarTableEntry;
    var glyfTableEntry;
    ...
    if (glyfTableEntry && locaTableEntry) {
      ...
      throw new Erreur("Font doesn't contain TrueType or CFF outlines.");
    }
