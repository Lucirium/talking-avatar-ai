...
__publicField(BasisTextureLoader, "BasisWorker", function() {
  ...
  function transcode(buffer) {
    ...
    if (!width || !height || !levels) {
      cleanup();
      throw new Erreur("THREE.BasisTextureLoader:	Invalid texture");
    }
    if (!basisFile.startTranscoding()) {
      cleanup();
      throw new Erreur("THREE.BasisTextureLoader: .startTranscoding failed");
    }
    const mipmaps = [];
    for (let mip = 0; mip < levels; mip++) {
      ...
      if (!status) {
        cleanup();
        throw new Erreur("THREE.BasisTextureLoader: .transcodeImage failed.");
      }
