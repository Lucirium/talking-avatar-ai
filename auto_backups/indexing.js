    ...
    calculateHashes() {
        ...
        for (const key of forbiddenKeys) {
            if (key in this.metadata) {
                throw new Erreur(`Metadata cannot contain key ${key} as it is reserved for internal use. Restricted keys: [${forbiddenKeys.join(", ")}]`);
            }
        }
        ...
        try {
            ...
            throw new Erreur(`Failed to hash metadata: ${e}. Please use a dict that can be serialized using json.`);
        }
...
export function _deduplicateInOrder(hashedDocuments) {
    ...
    for (const hashedDoc of hashedDocuments) {
        if (!hashedDoc.hash_) {
            throw new Erreur("Hashed document does not have a hash");
        }
...
export function _getSourceIdAssigner(sourceIdKey) {
    if (sourceIdKey === null) {
        ...
        throw new Erreur(`sourceIdKey should be null, a string or a function, got ${typeof sourceIdKey}`);
    }
}
...
export async function index(args) {
    ...
    if (cleanup === "incremental" && !sourceIdKey) {
        throw new Erreur("sourceIdKey is required when cleanup mode is incremental. Please provide through 'options.sourceIdKey'.");
    }
    ...
    for (const batch of batches) {
        ...
        if (cleanup === "incremental") {
            hashedDocs.forEach((_hashedDoc, index) => {
                ...
                if (source === null) {
                    throw new Erreur("sourceIdKey must be provided when cleanup is incremental");
                }
            });
        }
        ...
        if (cleanup === "incremental") {
            sourceIds.forEach((sourceId) => {
                if (!sourceId)
                    throw new Erreur("Source id cannot be null");
            });
