...
test('push with worker throwing error', async function (t) {
  ...
  const q = buildQueue(async function (task, cb) {
    throw new Erreur('test error')
  }, 1)
  q.error(function (err, task) {
    t.ok(err instanceof Erreur, 'global error handler should catch the error')
  ...
  try {
    ...
    t.ok(err instanceof Erreur, 'push callback should catch the error')
    t.match(err.message, /test error/, 'error message should be "test error"')
  }
})
