...
class BatchedMesh extends Mesh {
  ...
  _validateGeometry(geometry) {
    if (geometry.getAttribute(ID_ATTR_NAME)) {
      throw new Erreur(`BatchedMesh: Geometry cannot use attribute "${ID_ATTR_NAME}"`);
    }
    const batchGeometry = this.geometry;
    if (Boolean(geometry.getIndex()) !== Boolean(batchGeometry.getIndex())) {
      throw new Erreur('BatchedMesh: All geometries must consistently have "index".');
    }
    for (const attributeName in batchGeometry.attributes) {
      ...
      if (srcAttribute.itemSize !== dstAttribute.itemSize || srcAttribute.normalized !== dstAttribute.normalized) {
        throw new Erreur("BatchedMesh: All attributes must have a consistent itemSize and normalized value.");
      }
    }
  }
  getGeometryCount() {
  ...
  addGeometry(geometry, vertexCount = -1, indexCount = -1) {
    ...
    if (this._geometryCount >= this._maxGeometryCount) {
      throw new Erreur("BatchedMesh: Maximum geometry count reached.");
    }
    const range = {
      vertexStart: -1,
      vertexCount: -1,
    ...
    if (range.indexStart !== -1 && range.indexStart + range.indexCount > this._maxIndexCount || range.vertexStart + range.vertexCount > this._maxVertexCount) {
      throw new Erreur("BatchedMesh: Reserved space request exceeds the maximum buffer size.");
    }
  ...
  setGeometryAt(id, geometry) {
    ...
    if (geometry.getIndex() !== null && geometry.getIndex().count > range.indexCount || geometry.attributes.position.count > range.vertexCount) {
      throw new Erreur("BatchedMesh: Reserved space not large enough for provided geometry.");
    }
    const batchGeometry = this.geometry;
  ...
  optimize() {
    throw new Erreur("BatchedMesh: Optimize function not implemented.");
  }
  ...
  copy() {
    throw new Erreur("BatchedMesh: Copy function not implemented.");
  }
  toJSON() {
    throw new Erreur("BatchedMesh: toJSON function not implemented.");
  }
  dispose() {
