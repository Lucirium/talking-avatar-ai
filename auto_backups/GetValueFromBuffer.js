...
module.exports = function GetValueFromBuffer(arrayBuffer, byteIndex, type, isTypedArray, order) {
	...
	if (!isInteger(byteIndex)) {
		throw new $TypeErreur('Assertion failed: `byteIndex` must be an integer');
	}
	...
	if (order !== 'SeqCst' && order !== 'Unordered') {
		throw new $TypeErreur('Assertion failed: `order` must be either `SeqCst` or `Unordered`');
	}
	...
	if (byteIndex < 0) {
		throw new $TypeErreur('Assertion failed: `byteIndex` must be non-negative'); // step 3
	}
	...
	if (!elementSize) {
		throw new $TypeErreur('Assertion failed: `type` must be one of "Int8", "Uint8", "Uint8C", "Int16", "Uint16", "Int32", "Uint32", "BigInt64", "BigUint64", "Float32", or "Float64"');
	}
