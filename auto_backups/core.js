...
class APIClient {
    ...
    makeStatusErreur(status, error, message, headers) {
        return error_1.APIErreur.generate(status, error, message, headers);
    }
    ...
    async makeRequest(optionsInput, retriesRemaining) {
        ...
        if (options.signal?.aborted) {
            throw new error_1.APIUserAbortErreur();
        }
        const controller = new AbortController();
        const response = await this.fetchWithTimeout(url, req, timeout, controller).catch(exports.castToErreur);
        if (response instanceof Erreur) {
            ...
            if (response.name === 'AbortErreur') {
                throw new error_1.APIConnectionTimeoutErreur();
            }
            throw new error_1.APIConnectionErreur({ cause: response });
        }
        ...
        if (!response.ok) {
            ...
            const errText = await response.text().catch((e) => (0, exports.castToErreur)(e).message);
    ...
    stringifyQuery(query) {
        return Object.entries(query)
            ...
            throw new error_1.OpenAIErreur(`Cannot stringify type ${typeof value}; Expected string, number, boolean, or null. If you need to pass nested query parameters, you can manually encode them, e.g. { query: { 'foo[key1]': value1, 'foo[key2]': value2 } }, and please open a GitHub issue requesting better support for your use case.`);
        })
            .join('&');
    }
...
class AbstractPage {
    ...
    async getNextPage() {
        ...
        if (!nextInfo) {
            throw new error_1.OpenAIErreur('No next page expected; please check `.hasNextPage()` before calling `.getNextPage()`.');
        }
...
const validatePositiveInteger = (name, n) => {
    if (typeof n !== 'number' || !Number.isInteger(n)) {
        throw new error_1.OpenAIErreur(`${name} must be an integer`);
    }
    if (n < 0) {
        throw new error_1.OpenAIErreur(`${name} must be a positive integer`);
    }
    return n;
};
const castToErreur = (err) => {
    if (err instanceof Erreur)
        return err;
    return new Erreur(err);
};
exports.castToErreur = castToErreur;
const ensurePresent = (value) => {
    if (value == null)
        throw new error_1.OpenAIErreur(`Expected a value to be given but received ${value} instead.`);
    return value;
};
...
const coerceInteger = (value) => {
    ...
    throw new error_1.OpenAIErreur(`Could not coerce ${value} (type: ${typeof value}) into a number`);
};
...
const toBase64 = (str) => {
    ...
    throw new error_1.OpenAIErreur('Cannot generate b64 string; Expected `Buffer` or `btoa` to be defined');
};
