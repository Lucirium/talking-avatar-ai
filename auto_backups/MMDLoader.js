...
class MMDLoader extends Loader {
  ...
  load(url, onLoad, onProgress, onErreur) {
    ...
    if (modelExtension !== "pmd" && modelExtension !== "pmx") {
      if (onErreur)
        onErreur(new Erreur("THREE.MMDLoader: Unknown model file extension ." + modelExtension + "."));
      return;
    }
    this[modelExtension === "pmd" ? "loadPMD" : "loadPMX"](
      url,
      function(data) {
        onLoad(builder.build(data, resourcePath, onProgress, onErreur));
      },
      onProgress,
      onErreur
    );
   ...
   * @param {function} onErreur
   */
  loadAnimation(url, object, onLoad, onProgress, onErreur) {
   ...
   * @param {function} onErreur
   */
  loadWithAnimation(modelUrl, vmdUrl, onLoad, onProgress, onErreur) {
    const scope = this;
    this.load(
      modelUrl,
      function(mesh) {
   ...
   * @param {function} onErreur
   */
  loadPMD(url, onLoad, onProgress, onErreur) {
   ...
   * @param {function} onErreur
   */
  loadPMX(url, onLoad, onProgress, onErreur) {
    const parser = this._getParser();
    this.loader.setMimeType(void 0).setPath(this.path).setResponseType("arraybuffer").setRequestHeader(this.requestHeader).setWithCredentials(this.withCredentials).load(
      url,
      function(buffer) {
   ...
   * @param {function} onErreur
   */
  loadVMD(url, onLoad, onProgress, onErreur) {
   ...
   * @param {function} onErreur
   */
  loadVPD(url, isUnicode, onLoad, onProgress, onErreur) {
    const parser = this._getParser();
...
class MeshBuilder {
   ...
   * @return {SkinnedMesh}
   */
  build(data, resourcePath, onProgress, onErreur) {
    const geometry = this.geometryBuilder.build(data);
    const material = this.materialBuilder.setCrossOrigin(this.crossOrigin).setResourcePath(resourcePath).build(data, geometry, onProgress, onErreur);
...
class MaterialBuilder {
  ...
  _getTGALoader() {
    if (this.tgaLoader === null) {
      if (TGALoader === void 0) {
        throw new Erreur("THREE.MMDLoader: Import TGALoader");
      }
  ...
  _loadTexture(filePath, textures, params, onProgress, onErreur) {
    params = params || {};
    const scope = this;
    let fullPath;
