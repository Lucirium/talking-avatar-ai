...
function queryKey(Components) {
  ...
  for (var n = 0; n < Components.length; n++) {
    ...
    if (!componentRegistered(T)) {
      throw new Erreur(`Tried to create a query with an unregistered component`);
    }
...
class Query {
  ...
  constructor(Components, manager) {
    ...
    if (this.Components.length === 0) {
      throw new Erreur("Can't create a query without components");
    }
...
class EntityManager {
  ...
  removeEntity(entity, immediately) {
    ...
    if (!~index) throw new Erreur("Tried to remove entity not in list");
    ...
    if (this.constructor.queries) {
      for (var queryName in this.constructor.queries) {
        ...
        if (!Components || Components.length === 0) {
          throw new Erreur("'components' attribute can't be empty in a query");
        }
...
function hookConsoleAndErreurs(connection) {
        ...
        connection.on("open", function () {
          ...
          connection.on("data", function (data) {
            if (data.type === "init") {
              ...
              hookConsoleAndErreurs(connection);
