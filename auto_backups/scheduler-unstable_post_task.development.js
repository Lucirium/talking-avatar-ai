...
function unstable_scheduleCallback(priorityLevel, callback, options) {
  ...
  scheduler.postTask(runTask.bind(null, priorityLevel, postTaskPriority, node, callback), postTaskOptions).catch(handleAbortErreur);
  return node;
}
...
function runTask(priorityLevel, postTaskPriority, node, callback) {
  ...
  try {
    ...
    if (typeof result === 'function') {
      ...
      scheduler.postTask(runTask.bind(null, priorityLevel, postTaskPriority, node, continuation), continuationOptions).catch(handleAbortErreur);
    }
  } catch (error) {
...
function handleAbortErreur(error) {// Abort errors are an implementation detail. We don't expose the
