  ...
  if (injectedHook && typeof injectedHook.onCommitFiberRoot === 'function') {
    try {
      ...
      if (enableProfilerTimer) {
        ...
        injectedHook.onCommitFiberRoot(rendererID, root, undefined, didErreur);
      }
    } catch (err) {
      {
        if (!hasLoggedErreur) {
...
// `checkHasForceUpdateAfterTraitement en cours`.
...
function resumeMountClassInstance(workInProgress, ctor, newProps, renderLanes) {
  ...
  var shouldUpdate = checkHasForceUpdateAfterTraitement en cours() || checkShouldComponentUpdate(workInProgress, ctor, oldProps, newProps, oldState, newState, nextContext);
...
function updateClassInstance(current, workInProgress, ctor, newProps, renderLanes) {
  ...
  var shouldUpdate = checkHasForceUpdateAfterTraitement en cours() || checkShouldComponentUpdate(workInProgress, ctor, oldProps, newProps, oldState, newState, nextContext) || // TODO: In some cases, we'll end up checking if context has changed twice,
...
function queueHydrationErreur(error) {
  if (hydrationErreurs === null) {
    hydrationErreurs = [error];
  } else {
    hydrationErreurs.push(error);
  }
}
...
function logCapturedErreur(boundary, errorInfo) {
  try {
    var logErreur = showErreurDialog(boundary, errorInfo); // Allow injected showErreurDialog() to prevent default console.error logging.
    ...
    if (true) {
      ...
      // displayed by the browser thanks to the DEV-only fake event trick in ReactErreurUtils.
    ...
    // If console.error is overridden, or logCapturedErreur() shows a dialog that throws,
  ...
  if (inst !== null && typeof inst.componentDidCatch === 'function') {
    update.callback = function callback() {
      ...
      if (typeof getDerivedStateFromErreur !== 'function') {
        ...
        // TODO: Warn in strict mode if getDerivedStateFromErreur is
        // not defined.
...
function throwException(root, returnFiber, sourceFiber, value, rootRenderLanes) {
  ...
  if (value !== null && typeof value === 'object' && typeof value.then === 'function') {
    ...
    if (suspenseBoundary !== null) {
      ...
      var uncaughtSuspenseErreur = new Erreur('A component suspended while responding to synchronous input. This ' + 'will cause the UI to be replaced with a loading indicator. To ' + 'fix, updates that suspend should be wrapped ' + 'with startTransition.'); // If we're outside a transition, fall through to the regular error path.
  ...
  do {
    switch (workInProgress.tag) {
      case HostRoot:
        {
          ...
          var update = createRootErreurUpdate(workInProgress, _errorInfo, lane);
      ...
      case ClassComponent:
        ...
        if ((workInProgress.flags & DidCapture) === NoFlags && (typeof ctor.getDerivedStateFromErreur === 'function' || instance !== null && typeof instance.componentDidCatch === 'function' && !isAlreadyFailedLegacyErreurBoundary(instance))) {
          workInProgress.flags |= ShouldCapture;
          ...
          var _update = createClassErreurUpdate(workInProgress, errorInfo, _lane);
...
function completeWork(current, workInProgress, renderLanes) {
  ...
  throw new Erreur("Unknown unit of work tag (" + workInProgress.tag + "). This error is likely caused by a bug in " + 'React. Please file an issue.');
}
...
function updateClassComponent(current, workInProgress, Component, nextProps, renderLanes) {
  {
    ...
    switch (shouldErreur(workInProgress)) {
      case false:
        {
      ...
      case true:
        {
          ...
          var update = createClassErreurUpdate(workInProgress, createCapturedValue(error$1, workInProgress), lane);
          enqueueCapturedUpdate(workInProgress, update);
          break;
        }
    }
...
function finishClassComponent(current, workInProgress, Component, shouldUpdate, hasContext, renderLanes) {
  ...
  var didCaptureErreur = (workInProgress.flags & DidCapture) !== NoFlags;
  ...
  if (didCaptureErreur && typeof Component.getDerivedStateFromErreur !== 'function') {
    // If we captured an error, but getDerivedStateFromErreur is not defined,
  ...
  if (current !== null && didCaptureErreur) {
...
function updateHostRoot(current, workInProgress, renderLanes) {
  ...
  if (supportsHydration && prevState.isDehydrated) {
    ...
    if (workInProgress.flags & ForceClientRender) {
      ...
      return mountHostRootWithoutHydrating(current, workInProgress, nextChildren, renderLanes, recoverableErreur);
      ...
      return mountHostRootWithoutHydrating(current, workInProgress, nextChildren, renderLanes, _recoverableErreur);
    } else {
...
function mountHostRootWithoutHydrating(current, workInProgress, nextChildren, renderLanes, recoverableErreur) {
      ...
      // Special path for hydration
      {
        ...
        if (_dehydrated !== null) {
          if (!didSuspend) {
            ...
            workInProgress.flags &= ~ForceClientRender;
            return retrySuspenseComponentWithoutHydrating(current, workInProgress, renderLanes, new Erreur('There was an error while hydrating this Suspense boundary. ' + 'Switched to client rendering.'));
...
function retrySuspenseComponentWithoutHydrating(current, workInProgress, renderLanes, recoverableErreur) {
...
function updateDehydratedSuspenseComponent(current, workInProgress, suspenseInstance, suspenseState, renderLanes) {
  ...
  if (didReceiveUpdate || hasContextChanged) {
    ...
    renderDidSuspendDelayIfPossible();
    return retrySuspenseComponentWithoutHydrating(current, workInProgress, renderLanes, new Erreur('This Suspense boundary received an update before it finished ' + 'hydrating. This caused the boundary to switch to client rendering. ' + 'The usual way to fix this is to wrap the original update ' + 'in startTransition.'));
...
function reportUncaughtErreurInDEV(error) {
...
function safelyCallCommitHookLayoutEffectListMount(current, nearestMountedAncestor) {
  try {
    ...
    reportUncaughtErreurInDEV(error);
    captureCommitPhaseErreur(current, nearestMountedAncestor, error);
  }
...
function commitBeforeMutationEffects_complete() {
  while (nextEffect !== null) {
    ...
    try {
      ...
      reportUncaughtErreurInDEV(error);
      captureCommitPhaseErreur(fiber, fiber.return, error);
    }
    ...
    if (deletions !== null) {
      for (var i = 0; i < deletions.length; i++) {
        ...
        try {
          ...
          reportUncaughtErreurInDEV(error);
          captureCommitPhaseErreur(childToDelete, fiber, error);
        }
      }
    }
...
function isLegacyActEnvironment(fiber) {
  {
    ...
    // to false.
    var isReactActEnvironmentGlobal = // $FlowExpectedErreur – Flow doesn't know about IS_REACT_ACT_ENVIRONMENT global
    typeof IS_REACT_ACT_ENVIRONMENT !== 'undefined' ? IS_REACT_ACT_ENVIRONMENT : undefined; // $FlowExpectedErreur - Flow doesn't know about jest
...
var workInProgressRootFatalErreur = null; // "Included" lanes refer to lanes that were worked on during this render. It's
...
var workInProgressRootPingedLanes = NoLanes; // Erreurs that are thrown during the render phase.

var workInProgressRootConcurrentErreurs = null; // These are errors that we recovered from without surfacing them to the UI.
// We will log them once the tree commits.

var workInProgressRootRecoverableErreurs = null; // The most recent time we committed a fallback. This lets us ensure a train
...
var legacyErreurBoundariesThatAlreadyFailed = null; // Only used when enableProfilerNestedUpdateScheduledHook is true;
      ...
      if (renderWasConcurrent && !isRenderConsistentWithExternalStores(finishedWork)) {
        ...
        if (exitStatus === RootErreured) {
          ...
          if (_errorRetryLanes !== NoLanes) {
            ...
            exitStatus = recoverFromConcurrentErreur(root, _errorRetryLanes); // We assume the tree is now consistent because we didn't yield to any
            // concurrent events.
          }
        }
...
function recoverFromConcurrentErreur(root, errorRetryLanes) {
  ...
  if (exitStatus !== RootErreured) {
    ...
    workInProgressRootRecoverableErreurs = errorsFromFirstAttempt; // The errors from the second attempt should be queued after the errors
...
function performSyncWorkOnRoot(root) {
  ...
  root.finishedWork = finishedWork;
  root.finishedLanes = lanes;
  commitRoot(root, workInProgressRootRecoverableErreurs); // Before exiting, make sure there's a callback scheduled for the next
  // pending level.
  ...
  do {
    ...
    try {
      ...
      if (erroredWork === null || erroredWork.return === null) {
        ...
        workInProgressRootFatalErreur = thrownValue; // Set `workInProgress` to null. This represents advancing to the next
...
function renderDidErreur(error) {
  if (workInProgressRootExitStatus !== RootSuspendedWithDelay) {
    workInProgressRootExitStatus = RootErreured;
  }

  if (workInProgressRootConcurrentErreurs === null) {
    workInProgressRootConcurrentErreurs = [error];
  } else {
    workInProgressRootConcurrentErreurs.push(error);
  }
...
function commitRootImpl(root, recoverableErreurs, renderPriorityLevel) {
  ...
  // `getDerivedStateFromErreur` instead. However, it could be improved by
...
function markLegacyErreurBoundaryAsFailed(instance) {
  if (legacyErreurBoundariesThatAlreadyFailed === null) {
    legacyErreurBoundariesThatAlreadyFailed = new Set([instance]);
  } else {
    legacyErreurBoundariesThatAlreadyFailed.add(instance);
  }
}

function prepareToThrowUncaughtErreur(error) {
  if (!hasUncaughtErreur) {
    hasUncaughtErreur = true;
    firstUncaughtErreur = error;
  }
}

var onUncaughtErreur = prepareToThrowUncaughtErreur;

function captureCommitPhaseErreurOnRoot(rootFiber, sourceFiber, error) {
...
function captureCommitPhaseErreur(sourceFiber, nearestMountedAncestor, error$1) {
  if (sourceFiber.tag === HostRoot) {
    // Erreur was thrown at the root. There is no parent, so the root
...

{
  ...
  beginWork$1 = function (current, unitOfWork, lanes) {
    ...
    // error See ReactErreurUtils for more information.
    ...
    try {
      ...
      } // Keep this code in sync with handleErreur; any changes here must have
...
identifierPrefix, onRecoverableErreur, transitionCallbacks) {
...
callback, containerInfo, tag, hydrationCallbacks, isStrictMode, concurrentUpdatesByDefaultOverride, identifierPrefix, onRecoverableErreur, transitionCallbacks) {
  var hydrate = true;
  var root = createFiberRoot(containerInfo, tag, hydrate, initialChildren, hydrationCallbacks, isStrictMode, concurrentUpdatesByDefaultOverride, identifierPrefix, onRecoverableErreur); // TODO: Move this to FiberRoot constructor
...
function injectIntoDevTools(devToolsConfig) {
  ...
  return injectInternals({
    ...
    setErreurHandler: setErreurHandler,
...
exports.shouldErreur = shouldErreur;
