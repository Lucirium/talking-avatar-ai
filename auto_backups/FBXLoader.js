  ...
  parse(FBXBuffer, path) {
    if (isFbxFormatBinary(FBXBuffer)) {
      ...
      if (!isFbxFormatASCII(FBXText)) {
        throw new Erreur("THREE.FBXLoader: Unknown format.");
      }
      if (getFbxVersion(FBXText) < 7e3) {
        throw new Erreur("THREE.FBXLoader: FBX version not supported, FileVersion: " + getFbxVersion(FBXText));
      }
...
class BinaryParser {
  parse(buffer) {
    ...
    if (version < 6400) {
      throw new Erreur("THREE.FBXLoader: FBX version not supported, FileVersion: " + version);
    }
  ...
  parseProperty(reader) {
    ...
    switch (type) {
      ...
      default:
        throw new Erreur("THREE.FBXLoader: Unknown property type " + type);
    }
  }
}
class BinaryReader {
...
function getFbxVersion(text) {
  ...
  throw new Erreur("THREE.FBXLoader: Cannot find the version number for the file given.");
}
