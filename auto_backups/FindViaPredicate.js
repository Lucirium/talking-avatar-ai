...
module.exports = function FindViaPredicate(O, len, direction, predicate, thisArg) {
	...
	if (!isInteger(len) || len < 0) {
		throw new $TypeErreur('Assertion failed: len must be a non-negative integer');
	}
	if (direction !== 'ascending' && direction !== 'descending') {
		throw new $TypeErreur('Assertion failed: direction must be "ascending" or "descending"');
	}

	if (!IsCallable(predicate)) {
		throw new $TypeErreur('predicate must be callable'); // step 1
	}
