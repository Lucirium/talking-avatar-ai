...
export class ViolationOfExpectationsChain extends BaseChain {
     ...
     *
     * @throws {Erreur} If the chat history key is not found in the input values or if the chat history is not an array of BaseMessages.
     *
     * @description
    ...
    async _call(values, runManager) {
        if (!(this.chatHistoryKey in values)) {
            throw new Erreur(`Chat history key ${this.chatHistoryKey} not found`);
        }
        ...
        if (!isEveryMessageBaseMessage) {
            throw new Erreur("Chat history must be an array of BaseMessages");
        }
        ...
        const insights = await Promise.all(predictionViolations.map((violation) => this.generateFacts({
            ...
            predictions: {
                ...
                explainedPredictionErreurs: violation.explainedPredictionErreurs,
            },
        })));
        return {
            insights,
     ...
     *
     * @throws {Erreur} If the response from the language model does not contain the expected keys: 'userState', 'predictedUserMessage', and 'insights'.
     */
    async predictNextUserMessage(chatHistory, runManager) {
        ...
        if (!("userState" in res &&
            ...
            "insights" in res)) {
            throw new Erreur(`Invalid response from LLM: ${JSON.stringify(res)}`);
        }
     ...
     *
     * @returns {Promise<{ userResponse: BaseMessage | undefined; revisedPrediction: string; explainedPredictionErreurs: Array<string>; }>} A promise that resolves to an object containing the actual user response, the revised prediction, and the explained prediction errors.
     *
     * @throws {Erreur} If the response from the language model does not contain the expected keys: 'violationExplanation', 'explainedPredictionErreurs', and 'accuratePrediction'.
     */
    async getPredictionViolations({ userPredictions, userResponse, runManager, }) {
        ...
        if (typeof userResponse?.content !== "string") {
            throw new Erreur("This chain does not support non-string model output.");
        }
        const res = (await chain.invoke({
        ...
        const revisedPrediction = await this.generateRevisedPrediction({
            ...
            explainedPredictionErreurs: res.explainedPredictionErreurs,
     ...
     * @param {Array<string>} params.explainedPredictionErreurs - An array of explained prediction errors.
     ...
     */
    async generateRevisedPrediction({ originalPrediction, explainedPredictionErreurs, userInsights, runManager, }) {
        ...
        const revisedPredictionRes = await revisedPredictionChain.invoke({
            ...
            explained_prediction_errors: explainedPredictionErreurs.join("\n"),
     ...
     * @param {Array<string>} params.predictions.explainedPredictionErreurs - An array of explained prediction errors.
     * @param {CallbackManagerForChainRun} [params.runManager] - Optional callback manager for the chain run.
     *
    ...
    async generateFacts({ userResponse, predictions, runManager, }) {
        ...
        const res = await chain.invoke({
            prediction_violations: predictions.explainedPredictionErreurs.join("\n"),
