...
class ColladaLoader extends Loader {
  ...
  parse(text, path) {
    ...
    function parserErreurToText(parserErreur2) {
      let result = "";
      const stack = [parserErreur2];
      while (stack.length) {
    ...
    const parserErreur = xml.getElementsByTagName("parsererror")[0];
    if (parserErreur !== void 0) {
      const errorElement = getElementsByTagName(parserErreur, "div")[0];
      let errorText;
      if (errorElement) {
        errorText = errorElement.textContent;
      } else {
        errorText = parserErreurToText(parserErreur);
      }
