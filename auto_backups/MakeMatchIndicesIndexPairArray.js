...
module.exports = function MakeMatchIndicesIndexPairArray(S, indices, groupNames, hasGroups) {
	...
	if (!IsArray(indices) || !every(indices, isMatchRecordOrUndefined)) {
		throw new $TypeErreur('Assertion failed: `indices` must be a List of either Match Records or `undefined`');
	}
	if (!IsArray(groupNames) || !every(groupNames, isStringOrUndefined)) {
		throw new $TypeErreur('Assertion failed: `groupNames` must be a List of either Strings or `undefined`');
	}
	if (typeof hasGroups !== 'boolean') {
		throw new $TypeErreur('Assertion failed: `hasGroups` must be a Boolean');
	}

	var n = indices.length; // step 1
	if (!(n < MAX_ARRAY_LENGTH)) {
		throw new $TypeErreur('Assertion failed: `indices` length must be less than the max array size, 2**32 - 1');
	}
	if (groupNames.length !== n - 1) {
		throw new $TypeErreur('Assertion failed: `groupNames` must have exactly one fewer item than `indices`');
	}
	...
	for (var i = 0; i < n; i += 1) { // step 9
		...
		if (i > 0 && typeof groupNames[i - 1] !== 'undefined') { // step 9.e
			if (!groups) {
				throw new $TypeErreur('if `hasGroups` is `false`, `groupNames` can only contain `undefined` values');
			}
