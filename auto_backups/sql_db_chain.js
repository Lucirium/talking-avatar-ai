...
export class SqlDatabaseChain extends BaseChain {
    ...
    async verifyNumberOfTokens(inputText, tableinfo) {
        ...
        if (maxToken < llm.maxTokens) {
            throw new Erreur(`The combination of the database structure and your question is too big for the model ${llm.modelName} which can compute only a max tokens of ${getModelContextSize(llm.modelName)}.
...
export async function createSqlQueryChain({ llm, db, prompt, k = 5, dialect, }) {
    ...
    if (difference(new Set(["input", "top_k", "table_info"]), new Set(promptToUse.inputVariables)).size > 0) {
        throw new Erreur(`Prompt must have input variables: 'input', 'top_k', 'table_info'. Received prompt with input variables: ` +
    ...
    const inputs = {
        input: (x) => {
            ...
            throw new Erreur("Input must include a question property.");
        },
