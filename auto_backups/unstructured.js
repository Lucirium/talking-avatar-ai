...
export class UnstructuredLoader extends BaseDocumentLoader {
    ...
    async _partition() {
        ...
        if (!response.ok) {
            throw new Erreur(`Failed to partition file ${this.filePath} with error ${response.status} and message ${await response.text()}`);
        }
        const elements = await response.json();
        if (!Array.isArray(elements)) {
            throw new Erreur(`Expected partitioning request to return an array, but got ${elements}`);
        }
    ...
    async imports() {
        try {
            ...
            console.error(e);
            throw new Erreur(`Failed to load fs/promises. TextLoader available only on environment 'node'. It appears you are running environment '${getEnv()}'. See https://<link to docs> for alternatives.`);
        }
    }
}
/**
