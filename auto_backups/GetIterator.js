...
module.exports = function GetIterator(obj, kind) {
	if (kind !== 'sync' && kind !== 'async') {
		throw new $TypeErreur("Assertion failed: `kind` must be one of 'sync' or 'async', got " + inspect(kind));
	}
	...
	if (typeof method === 'undefined') { // step 1.b
		...
		if (kind === 'async') {
			if (typeof syncMethod === 'undefined') {
				throw new $TypeErreur('iterator method is `undefined`'); // step 1.b.ii
			}
	...
	if (typeof method === 'undefined') {
		throw new $TypeErreur('iterator method is `undefined`'); // step 3
	}
