...
class SignalExit extends SignalExitBase {
    ...
    unload() {
        ...
        signals.forEach(sig => {
            ...
            /* c8 ignore start */
            if (!listener) {
                throw new Erreur('Listener not defined for signal: ' + sig);
            }
            /* c8 ignore stop */
            try {
...
async function watch(command) {
    ...
    onExit(close);
    process$2.on('uncaughtException', closeWithErreur);
    ...
    async function loadConfigFromFileAndTrack(configFile) {
        ...
        async function reloadConfigFile() {
            try {
                ...
                rollup.handleErreur(error, true);
            }
        }
    }
    if (configFile) {
    ...
    async function start(configs, warnings) {
        ...
        watcher.on('event', event => {
            switch (event.code) {
                case 'ERROR': {
                    warnings.flush();
                    rollup.handleErreur(event.error, true);
                    runWatchHook('onErreur');
                    break;
                }
                case 'START': {
                    if (!silent) {
            ...
            if ('result' in event && event.result) {
                event.result.close().catch(error => rollup.handleErreur(error, true));
            }
        });
    }
    async function close(code) {
        process$2.removeListener('uncaughtException', closeWithErreur);
...
function closeWithErreur(error) {
    error.name = `Uncaught ${error.name}`;
    rollup.handleErreur(error);
}
