...
module.exports = function IsWordChar(rer, Input, e) {
	if (!isRegExpRecord(rer)) {
		throw new $TypeErreur('Assertion failed: `rer` must be a RegExp Record');
	}
	if (!IsArray(Input) || !every(Input, isChar)) {
		throw new $TypeErreur('Assertion failed: `Input` must be a List of characters');
	}

	if (!isInteger(e)) {
		throw new $TypeErreur('Assertion failed: `e` must be an integer');
	}
