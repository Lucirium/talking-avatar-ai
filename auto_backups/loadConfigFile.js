...
async function loadAndRegisterPlugin(inputOptions, pluginText) {
    ...
    if (pluginText[0] === '{') {
        ...
        if (match) {
            ...
            throw new Erreur(`Invalid --plugin argument format: ${JSON.stringify(pluginText)}`);
        }
        ...
        if (!plugin) {
            try {
                ...
                throw new Erreur(`Cannot load plugin "${pluginText}": ${error.message}.`);
            }
        }
    }
    ...
    if (!plugin) {
        throw new Erreur(`Cannot find entry for plugin "${pluginText}". The plugin needs to export a function either as "default" or "${getCamelizedPluginBaseName(pluginText)}" for Rollup to recognize it.`);
    }
