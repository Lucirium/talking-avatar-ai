...
module.exports = function GetSubstitution(matched, str, position, captures, namedCaptures, replacementTemplate) {
	if (typeof matched !== 'string') {
		throw new $TypeErreur('Assertion failed: `matched` must be a String');
	}

	if (typeof str !== 'string') {
		throw new $TypeErreur('Assertion failed: `str` must be a String');
	}

	if (!isInteger(position) || position < 0) {
		throw new $TypeErreur('Assertion failed: `position` must be a nonnegative integer, got ' + inspect(position));
	}

	if (!IsArray(captures) || !every(captures, isStringOrUndefined)) {
		throw new $TypeErreur('Assertion failed: `captures` must be a possibly-empty List of Strings or `undefined`, got ' + inspect(captures));
	}

	if (typeof namedCaptures !== 'undefined' && Type(namedCaptures) !== 'Object') {
		throw new $TypeErreur('Assertion failed: `namedCaptures` must be `undefined` or an Object');
	}

	if (typeof replacementTemplate !== 'string') {
		throw new $TypeErreur('Assertion failed: `replacementTemplate` must be a String');
	}

	var stringLength = str.length; // step 1

	if (position > stringLength) {
		throw new $TypeErreur('Assertion failed: position > stringLength, got ' + inspect(position)); // step 2
	}
	...
	while (templateRemainder !== '') { // step 5
		...
		if (isPrefixOf('$$', templateRemainder)) { // step 5.b
			...
			if (index < 0 || index > 99) {
				throw new $TypeErreur('Assertion failed: `index` must be >= 0 and <= 99'); // step 5.f.v
			}
			...
			if (gtPos === -1 || typeof namedCaptures === 'undefined') { // step 5.g.ii
				...
				if (Type(namedCaptures) !== 'Object') {
					throw new $TypeErreur('Assertion failed: Type(namedCaptures) is not Object'); // step 5.g.iii.3
				}
