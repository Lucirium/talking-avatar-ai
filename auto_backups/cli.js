...
class CACErreur extends Erreur {
  constructor(message) {
    super(message);
    this.name = this.constructor.name;
    if (typeof Erreur.captureStackTrace === "function") {
      Erreur.captureStackTrace(this, this.constructor);
    } else {
      this.stack = new Erreur(message).stack;
    }
  }
}
...
${section.body}` : section.body;
  ...
  checkRequiredArgs() {
    ...
    if (this.cli.args.length < minimalArgsCount) {
      throw new CACErreur(`missing required args for command \`${this.rawName}\``);
    }
  }
  checkUnknownOptions() {
    ...
    if (!this.config.allowUnknownOptions) {
      for (const name of Object.keys(options)) {
        if (name !== "--" && !this.hasOption(name) && !globalCommand.hasOption(name)) {
          throw new CACErreur(`Unknown option \`${name.length > 1 ? `--${name}` : `-${name}`}\``);
        }
      }
    }
  }
    ...
    for (const option of options) {
      ...
      if (option.required) {
        ...
        if (value === true || value === false && !hasNegated) {
          throw new CACErreur(`option \`${option.rawName}\` value is missing`);
        }
      }
    }
  }
...
cli
    ...
    try {
        ...
        if (!server.httpServer) {
            throw new Erreur('HTTP server not available');
        }
        await server.listen();
