...
module.exports = function ArraySetLength(A, Desc) {
	if (!IsArray(A)) {
		throw new $TypeErreur('Assertion failed: A must be an Array');
	}
	...
	if (newLen !== numberLen) {
		throw new $RangeErreur('Invalid array length');
	}
	newLenDesc['[[Value]]'] = newLen;
	var oldLenDesc = OrdinaryGetOwnProperty(A, 'length');
	if (!IsDataDescriptor(oldLenDesc)) {
		throw new $TypeErreur('Assertion failed: an array had a non-data descriptor on `length`');
	}
