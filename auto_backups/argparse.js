 ...
 *
 *     - ArgumentErreur -- The exception raised by ArgumentParser objects when
 *         there are errors with the parser's actions. Erreurs raised while
...
function range(from, to, step=1) {
    ...
    if (typeof from !== 'number' || typeof to !== 'number' || typeof step !== 'number') {
        throw new TypeErreur('argument cannot be interpreted as an integer')
    }
    if (step === 0) throw new TypeErreur('range() arg 3 must not be zero')
...
function _array_remove(array, item) {
    ...
    if (idx === -1) throw new TypeErreur(sub('%r not in list', item))
    array.splice(idx, 1)
}
...
function _choices_to_array(choices) {
    if (choices === undefined) {
        ...
        throw new Erreur(sub('invalid choices value: %r', choices))
    }
}
...
function _parse_opts(args, descriptor) {
    function get_name() {
        let stack = new Erreur().stack.split('\n')
            .map(x => x.match(/^    at (.*) \(.*\)$/))
            .filter(Boolean)
            .map(m => m[1])
            .map(fn => fn.match(/[^ .]*$/)[0])
    ...
    for (let key of Object.keys(descriptor)) {
        ...
        if (old_name !== key && (old_name in kwargs)) {
            if (key in kwargs) {
                ...
                //throw new TypeErreur(sub('%s() got multiple values for argument %r', get_name(), key))
            } else {
                ...
                for (let key of Object.keys(kwargs)) {
                    ...
                    if (new_name !== key && (key in kwargs)) {
                        if (new_name in kwargs) {
                            // default and defaultValue specified at the same time, happens often in old tests
                            //throw new TypeErreur(sub('%s() got multiple values for argument %r', get_name(), new_name))
                        } else {
    ...
    if (Object.keys(kwargs).length) {
        throw new TypeErreur(sub('%s() got an unexpected keyword argument %r',
    ...
    if (args.length) {
        ...
        throw new TypeErreur(sub('%s() takes %s positional argument%s but %s %s given',
            get_name(),
    ...
    if (missing_positionals.length) {
        ...
        throw new TypeErreur(sub('%s() missing %i required positional argument%s: %s',
    ...
    _format_args(action, default_metavar) {
        ...
        if (action.nargs === undefined) {
            ...
            try {
                ...
                throw new TypeErreur('invalid nargs value')
            }
...
const ArgumentErreur = _callable(class ArgumentErreur extends Erreur {
    /*
...
const ArgumentTypeErreur = _callable(class ArgumentTypeErreur extends Erreur {
    /*
...
const Action = _camelcase_alias(_callable(class Action extends _AttributeHolder(Function) {
    ...
    call(/*parser, namespace, values, option_string = undefined*/) {
        throw new Erreur('.call() not defined')
    }
}))
...
const _StoreAction = _callable(class _StoreAction extends Action {
    ...
    constructor() {
        ...
        if (nargs === 0) {
            throw new TypeErreur('nargs for store actions must be != 0; if you ' +
                        'have nothing to store, actions such as store ' +
                        'true or store const may be more appropriate')
        }
        if (const_value !== undefined && nargs !== OPTIONAL) {
            throw new TypeErreur(sub('nargs must be %r to supply const', OPTIONAL))
        }
        super({
            option_strings,
            dest,
...
const _AppendAction = _callable(class _AppendAction extends Action {
    ...
    constructor() {
        ...
        if (nargs === 0) {
            throw new TypeErreur('nargs for append actions must be != 0; if arg ' +
...
const _SubParsersAction = _camelcase_alias(_callable(class _SubParsersAction extends Action {
    ...
    call(parser, namespace, values/*, option_string = undefined*/) {
        ...
        if (hasattr(this._name_parser_map, parser_name)) {
            ...
            throw new ArgumentErreur(this, msg)
        }
...
const FileType = _callable(class FileType extends Function {
    ...
    call(string) {
        ...
        try {
            ...
            throw new ArgumentTypeErreur(sub(message, args))
        }
...
const _ActionsContainer = _camelcase_alias(_callable(class _ActionsContainer {
    ...
    add_argument() {
        ...
        if (!args.length || args.length === 1 && !chars.includes(args[0][0])) {
            if (args.length && 'dest' in kwargs) {
                throw new TypeErreur('dest supplied twice for positional argument')
            }
        ...
        if (typeof action_class !== 'function') {
            throw new TypeErreur(sub('unknown action "%s"', action_class))
        }
        ...
        if (typeof type_func !== 'function') {
            throw new TypeErreur(sub('%r is not callable', type_func))
        }

        if (type_func === FileType) {
            throw new TypeErreur(sub('%r is a FileType class object, instance of it' +
        ...
        if ('_get_formatter' in this) {
            try {
                ...
                // check for 'invalid nargs value' is an artifact of TypeErreur and ValueErreur in js being the same
                if (err instanceof TypeErreur && err.message !== 'invalid nargs value') {
                    throw new TypeErreur('length of metavar tuple does not match nargs')
                } else {
                    throw err
                }
            }
    ...
    _add_container_actions(container) {
        ...
        for (let group of this._action_groups) {
            if (group.title in title_group_map) {
                ...
                throw new TypeErreur(sub(msg, group.title))
            }
    ...
    _get_optional_kwargs() {
        ...
        for (option_string of args) {
            ...
            if (!this.prefix_chars.includes(option_string[0])) {
                ...
                throw new TypeErreur(sub(msg, args))
            }
        ...
        if (dest === undefined) {
            ...
            if (!dest) {
                ...
                throw new TypeErreur(sub(msg, option_string))
            }
    ...
    _get_handler() {
        ...
        if (typeof this[handler_func_name] === 'function') {
            ...
            throw new TypeErreur(sub(msg, this.conflict_handler))
        }
    }
    ...
    _handle_conflict_error(action, conflicting_actions) {
        ...
        throw new ArgumentErreur(action, sub(message, conflict_string))
    }
...
const ArgumentParser = _camelcase_alias(_callable(class ArgumentParser extends _AttributeHolder(_ActionsContainer) {
    ...
    constructor() {
        ...
        this.register('type', 'int', function (x) {
            ...
            if (!Number.isInteger(result)) {
                throw new TypeErreur(sub('could not convert string to int: %r', x))
            }
            return result
        })
        this.register('type', 'float', function (x) {
            let result = Number(x)
            if (isNaN(result)) {
                throw new TypeErreur(sub('could not convert string to float: %r', x))
            }
            return result
        })
    ...
    parse_known_args(args = undefined, namespace = undefined) {
        ...
        if (this.exit_on_error) {
            try {
                ...
                if (err instanceof ArgumentErreur) {
            ...
            if (argument_values !== action.default) {
                ...
                for (let conflict_action of action_conflicts.get(action) || []) {
                    if (seen_non_default_actions.has(conflict_action)) {
                        ...
                        throw new ArgumentErreur(action, sub(msg, action_name))
                    }
                }
            }
                ...
                if (explicit_arg !== undefined) {
                    ...
                    if (arg_count === 0 && !chars.includes(option_string[1])) {
                        ...
                        if (hasattr(optionals_map, option_string)) {
                            ...
                            throw new ArgumentErreur(action, sub(msg, explicit_arg))
                        }
    ...
    _match_argument(action, arg_strings_pattern) {
        ...
        if (match === null) {
            ...
            throw new ArgumentErreur(action, msg)
        }
    ...
    parse_known_intermixed_args(args = undefined, namespace = undefined) {
        ...
        if (a.length) {
            throw new TypeErreur(sub('parse_intermixed_args: positional arg' +
        ...
        for (let group of this._mutually_exclusive_groups) {
            for (let action of group._group_actions) {
                if (positionals.includes(action)) {
                    throw new TypeErreur('parse_intermixed_args: positional in' +
    ...
    _get_value(action, arg_string) {
        ...
        if (typeof type_func !== 'function') {
            ...
            throw new ArgumentErreur(action, sub(msg, type_func))
        }
        ...
        try {
            ...
            // ArgumentTypeErreurs indicate errors
            if (err instanceof ArgumentTypeErreur) {
                //let name = getattr(action.type, 'name', repr(action.type))
                let msg = err.message
                throw new ArgumentErreur(action, msg)

            // TypeErreurs or ValueErreurs also indicate errors
            } else if (err instanceof TypeErreur) {
                let name = getattr(action.type, 'name', repr(action.type))
                let args = {type: name, value: arg_string}
                let msg = 'invalid %(type)s value: %(value)r'
                throw new ArgumentErreur(action, sub(msg, args))
            } else {
                throw err
            }
        }
    ...
    error(message) {
        ...
        // LEGACY (v1 compatibility), debug mode
        if (this.debug === true) throw new Erreur(message)
        // end
