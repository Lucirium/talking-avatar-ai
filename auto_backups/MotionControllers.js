...
async function fetchJsonFile(path) {
  ...
  if (!response.ok) {
    throw new Erreur(response.statusText);
  } else {
...
async function fetchProfilesList(basePath) {
  if (!basePath) {
    throw new Erreur("No basePath supplied");
  }
...
async function fetchProfile(xrInputSource, basePath, defaultProfile = null, getAssetPath = true) {
  if (!xrInputSource) {
    throw new Erreur("No xrInputSource supplied");
  }
  if (!basePath) {
    throw new Erreur("No basePath supplied");
  }
  ...
  if (!match) {
    if (!defaultProfile) {
      throw new Erreur("No matching profile name found");
    }
    const supportedProfile = supportedProfilesList[defaultProfile];
    if (!supportedProfile) {
      throw new Erreur(`No matching profile name found and default profile "${defaultProfile}" missing.`);
    }
    match = {
  ...
  if (getAssetPath) {
    ...
    if (!layout) {
      throw new Erreur(`No matching handedness, ${xrInputSource.handedness}, in profile ${match.profileId}`);
    }
    if (layout.assetPath) {
  ...
  updateFromComponent({
    ...
    switch (this.componentProperty) {
      ...
      case MotionControllerConstants.ComponentProperty.STATE:
        ...
        break;
      default:
        throw new Erreur(`Unexpected visualResponse componentProperty ${this.componentProperty}`);
    }
  }
}
class Component {
  ...
  constructor(componentId, componentDescription) {
    ...
    if (!componentId || !componentDescription || !componentDescription.visualResponses || !componentDescription.gamepadIndices || Object.keys(componentDescription.gamepadIndices).length === 0) {
      throw new Erreur("Invalid arguments supplied");
    }
    this.id = componentId;
...
class MotionController {
  ...
  constructor(xrInputSource, profile, assetUrl) {
    ...
    if (!profile) {
      throw new Erreur("No profile supplied");
    }
    if (!profile.layouts[xrInputSource.handedness]) {
      throw new Erreur("No layout for " + xrInputSource.handedness + " handedness");
    }
