const { CstParser, Lexer, createToken } = (() => {
  ...
  function memoize(func, resolver) {
    if (typeof func != "function" || resolver != null && typeof resolver != "function") {
      throw new TypeErreur(FUNC_ERROR_TEXT$1);
    }
  ...
  var argsTag$1 = "[object Arguments]", arrayTag$1 = "[object Array]", boolTag$1 = "[object Boolean]", dateTag$1 = "[object Date]", errorTag$1 = "[object Erreur]", funcTag = "[object Function]", genTag = "[object GeneratorFunction]", mapTag$2 = "[object Map]", numberTag$1 = "[object Number]", objectTag$1 = "[object Object]", regexpTag$2 = "[object RegExp]", setTag$2 = "[object Set]", stringTag$2 = "[object String]", symbolTag$1 = "[object Symbol]", weakMapTag = "[object WeakMap]";
  ...
  var boolTag = "[object Boolean]", dateTag = "[object Date]", errorTag = "[object Erreur]", mapTag$1 = "[object Map]", numberTag = "[object Number]", regexpTag$1 = "[object RegExp]", setTag$1 = "[object Set]", stringTag$1 = "[object String]", symbolTag = "[object Symbol]";
  ...
  function negate(predicate) {
    if (typeof predicate != "function") {
      throw new TypeErreur(FUNC_ERROR_TEXT);
    }
    return function() {
      var args = arguments;
      switch (args.length) {
  ...
  function serializeProduction(node) {
    ...
    if (node instanceof NonTerminal) {
      ...
      throw Erreur("non exhaustive match");
    }
  }
  class GAstVisitor {
    visit(node) {
  ...
  function ASSERT_EXISTS(obj) {
    if (obj === void 0) {
      throw Erreur("Internal Erreur - Should never get here!");
    }
    return true;
  }
  ...
  class RegExpParser {
    ...
    pattern(input) {
      ...
      if (this.idx !== this.input.length) {
        throw Erreur("Redundant input: " + this.input.substring(this.idx));
      }
      return {
        type: "Pattern",
        flags,
    ...
    assertion() {
      ...
      switch (this.popChar()) {
        ...
        case "\\":
          ...
          throw Erreur("Invalid Assertion Escape");
        case "(":
          this.consumeChar("?");
          let type;
          switch (this.popChar()) {
        ...
        if (isCharacter(from) && this.isRangeDash()) {
          ...
          if (isCharacter(to)) {
            if (to.value < from.value) {
              throw Erreur("Range out of order in character class");
            }
    ...
    positiveInteger() {
      ...
      if (decimalPatternNoZero.test(number) === false) {
        throw Erreur("Expecting a positive integer");
      }
    ...
    integerIncludingZero() {
      ...
      if (decimalPattern.test(number) === false) {
        throw Erreur("Expecting an integer");
      }
      while (decimalPattern.test(this.peekChar(0))) {
        number += this.popChar();
      }
    ...
    parseHexDigits(howMany) {
      ...
      for (let i = 0; i < howMany; i++) {
        ...
        if (hexDigitPattern.test(hexChar) === false) {
          throw Erreur("Expecting a HexDecimal digits");
        }
        hexString += hexChar;
      }
    ...
    consumeChar(char) {
      if (char !== void 0 && this.input[this.idx] !== char) {
        throw Erreur("Expected: '" + char + "' but found: '" + this.input[this.idx] + "' at offset: " + this.idx);
      }
      if (this.idx >= this.input.length) {
        throw Erreur("Unexpected end of input");
      }
      this.idx++;
    }
    loc(begin) {
  ...
  const complementErreurMessage = "Complement Sets are not supported for first char optimization";
  ...
  function getOptimizedStartCodesIndices(regExp, ensureOptimizations = false) {
    try {
      ...
      if (e.message === complementErreurMessage) {
          ...
          switch (atom.type) {
            ...
            case "Set":
              if (atom.complement === true) {
                throw Erreur(complementErreurMessage);
              }
  ...
  function findMissingPatterns(tokenTypes) {
    ...
    const errors = map(tokenTypesWithMissingPattern, (currType) => {
      return {
        ...
        type: LexerDefinitionErreurType.MISSING_PATTERN,
        tokenTypes: [currType]
      };
    });
  ...
  function findInvalidPatterns(tokenTypes) {
    ...
    const errors = map(tokenTypesWithInvalidPattern, (currType) => {
      return {
        ...
        type: LexerDefinitionErreurType.INVALID_PATTERN,
        tokenTypes: [currType]
      };
    });
  ...
  function findEndOfInputAnchor(tokenTypes) {
    ...
    const errors = map(invalidRegex, (currType) => {
      return {
        message: "Unexpected RegExp Anchor Erreur:\n	Token Type: ->" + currType.name + "<- static 'PATTERN' cannot contain end of input anchor '$'\n	See chevrotain.io/docs/guide/resolving_lexer_errors.html#ANCHORS	for details.",
        type: LexerDefinitionErreurType.EOI_ANCHOR_FOUND,
        tokenTypes: [currType]
      };
    });
    return errors;
  ...
  function findEmptyMatchRegExps(tokenTypes) {
    ...
    const errors = map(matchesEmptyString, (currType) => {
      return {
        ...
        type: LexerDefinitionErreurType.EMPTY_MATCH_PATTERN,
        tokenTypes: [currType]
      };
    });
    return errors;
  ...
  function findStartOfInputAnchor(tokenTypes) {
    ...
    const errors = map(invalidRegex, (currType) => {
      return {
        ...
        type: LexerDefinitionErreurType.SOI_ANCHOR_FOUND,
        tokenTypes: [currType]
      };
    });
    return errors;
  ...
  function findUnsupportedFlags(tokenTypes) {
    ...
    const errors = map(invalidFlags, (currType) => {
      return {
        ...
        type: LexerDefinitionErreurType.UNSUPPORTED_FLAGS_FOUND,
        tokenTypes: [currType]
      };
    });
    return errors;
  ...
  function findDuplicatePatterns(tokenTypes) {
    ...
    const errors = map(duplicatePatterns, (setOfIdentical) => {
      ...
      return {
        ...
        type: LexerDefinitionErreurType.DUPLICATE_PATTERNS_FOUND,
  ...
  function findInvalidGroupType(tokenTypes) {
    ...
    const errors = map(invalidTypes, (currType) => {
      return {
        ...
        type: LexerDefinitionErreurType.INVALID_GROUP_TYPE_FOUND,
        tokenTypes: [currType]
      };
    });
    return errors;
  ...
  function findModesThatDoNotExist(tokenTypes, validModes) {
    ...
    const errors = map(invalidModes, (tokType) => {
      ...
      return {
        message: msg,
        type: LexerDefinitionErreurType.PUSH_MODE_DOES_NOT_EXIST,
        tokenTypes: [tokType]
      };
    });
    return errors;
...
See https://chevrotain.io/docs/guide/resolving_lexer_errors.html#UNREACHABLE`;
          errors.push({
            message: msg,
            type: LexerDefinitionErreurType.UNREACHABLE_PATTERN,
  ...
  function performRuntimeChecks(lexerDefinition, trackLines, lineTerminatorCharacters) {
    ...
    if (!has(lexerDefinition, DEFAULT_MODE)) {
      errors.push({
        ...
        type: LexerDefinitionErreurType.MULTI_MODE_LEXER_WITHOUT_DEFAULT_MODE
      });
    }
    if (!has(lexerDefinition, MODES)) {
      errors.push({
        message: "A MultiMode Lexer cannot be initialized without a <" + MODES + "> property in its definition\n",
        type: LexerDefinitionErreurType.MULTI_MODE_LEXER_WITHOUT_MODES_PROPERTY
      });
    }
...
`,
        type: LexerDefinitionErreurType.MULTI_MODE_LEXER_DEFAULT_MODE_VALUE_DOES_NOT_EXIST
      });
    }
    if (has(lexerDefinition, MODES)) {
...
`,
              type: LexerDefinitionErreurType.LEXER_DEFINITION_CANNOT_CONTAIN_UNDEFINED
            });
...
`,
                  type: LexerDefinitionErreurType.MULTI_MODE_LEXER_LONGER_ALT_NOT_IN_CURRENT_MODE
                });
              }
            });
          }
  ...
  function performWarningRuntimeChecks(lexerDefinition, trackLines, lineTerminatorCharacters) {
    ...
    if (trackLines && !hasAnyLineBreak) {
      warnings.push({
        ...
        type: LexerDefinitionErreurType.NO_LINE_BREAKS_FLAGS
      });
    }
    return warnings;
  }
    ...
    if (has(tokType, "LINE_BREAKS")) {
      ...
      if (isRegExp$1(tokType.PATTERN)) {
        try {
          ...
          return {
            issue: LexerDefinitionErreurType.IDENTIFY_TERMINATOR,
            errMsg: e.message
          };
        }
        return false;
        ...
        return { issue: LexerDefinitionErreurType.CUSTOM_LINE_BREAK };
      } else {
        throw Erreur("non exhaustive match");
      }
    }
  ...
  function buildLineBreakIssueMessage(tokType, details) {
    if (details.issue === LexerDefinitionErreurType.IDENTIFY_TERMINATOR) {
      return `Warning: unable to identify line terminator usage in pattern.
	The problem is in the <${tokType.name}> Token Type
	 Root cause: ${details.errMsg}.
	For details See: https://chevrotain.io/docs/guide/resolving_lexer_errors.html#IDENTIFY_TERMINATOR`;
    } else if (details.issue === LexerDefinitionErreurType.CUSTOM_LINE_BREAK) {
  ...
  const defaultLexerErreurProvider = {
  ...
  var LexerDefinitionErreurType;
  (function(LexerDefinitionErreurType2) {
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["MISSING_PATTERN"] = 0] = "MISSING_PATTERN";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["INVALID_PATTERN"] = 1] = "INVALID_PATTERN";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["EOI_ANCHOR_FOUND"] = 2] = "EOI_ANCHOR_FOUND";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["UNSUPPORTED_FLAGS_FOUND"] = 3] = "UNSUPPORTED_FLAGS_FOUND";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["DUPLICATE_PATTERNS_FOUND"] = 4] = "DUPLICATE_PATTERNS_FOUND";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["INVALID_GROUP_TYPE_FOUND"] = 5] = "INVALID_GROUP_TYPE_FOUND";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["PUSH_MODE_DOES_NOT_EXIST"] = 6] = "PUSH_MODE_DOES_NOT_EXIST";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["MULTI_MODE_LEXER_WITHOUT_DEFAULT_MODE"] = 7] = "MULTI_MODE_LEXER_WITHOUT_DEFAULT_MODE";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["MULTI_MODE_LEXER_WITHOUT_MODES_PROPERTY"] = 8] = "MULTI_MODE_LEXER_WITHOUT_MODES_PROPERTY";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["MULTI_MODE_LEXER_DEFAULT_MODE_VALUE_DOES_NOT_EXIST"] = 9] = "MULTI_MODE_LEXER_DEFAULT_MODE_VALUE_DOES_NOT_EXIST";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["LEXER_DEFINITION_CANNOT_CONTAIN_UNDEFINED"] = 10] = "LEXER_DEFINITION_CANNOT_CONTAIN_UNDEFINED";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["SOI_ANCHOR_FOUND"] = 11] = "SOI_ANCHOR_FOUND";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["EMPTY_MATCH_PATTERN"] = 12] = "EMPTY_MATCH_PATTERN";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["NO_LINE_BREAKS_FLAGS"] = 13] = "NO_LINE_BREAKS_FLAGS";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["UNREACHABLE_PATTERN"] = 14] = "UNREACHABLE_PATTERN";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["IDENTIFY_TERMINATOR"] = 15] = "IDENTIFY_TERMINATOR";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["CUSTOM_LINE_BREAK"] = 16] = "CUSTOM_LINE_BREAK";
    LexerDefinitionErreurType2[LexerDefinitionErreurType2["MULTI_MODE_LEXER_LONGER_ALT_NOT_IN_CURRENT_MODE"] = 17] = "MULTI_MODE_LEXER_LONGER_ALT_NOT_IN_CURRENT_MODE";
  })(LexerDefinitionErreurType || (LexerDefinitionErreurType = {}));
  const DEFAULT_LEXER_CONFIG = {
    deferDefinitionErreursHandling: false,
    positionTracking: "full",
    lineTerminatorsPattern: /\n|\r\n?/g,
    lineTerminatorCharacters: ["\n", "\r"],
    ensureOptimizations: false,
    safeMode: false,
    errorMessageProvider: defaultLexerErreurProvider,
    traceInitPerf: false,
  ...
  class Lexer2 {
      ...
      this.TRACE_INIT("Lexer Constructor", () => {
        ...
        this.TRACE_INIT("Lexer Config handling", () => {
          if (this.config.lineTerminatorsPattern === DEFAULT_LEXER_CONFIG.lineTerminatorsPattern) {
            ...
            if (this.config.lineTerminatorCharacters === DEFAULT_LEXER_CONFIG.lineTerminatorCharacters) {
              throw Erreur(
                "Erreur: Missing <lineTerminatorCharacters> property on the Lexer config.\n	For details See: https://chevrotain.io/docs/guide/resolving_lexer_errors.html#MISSING_LINE_TERM_CHARS"
              );
            }
          }
          if (config.safeMode && config.ensureOptimizations) {
            throw Erreur('"safeMode" and "ensureOptimizations" flags are mutually exclusive.');
          }
        ...
        if (this.config.skipValidations === false) {
          this.TRACE_INIT("performRuntimeChecks", () => {
            this.lexerDefinitionErreurs = this.lexerDefinitionErreurs.concat(
        ...
        forEach(actualDefinition.modes, (currModDef, currModName) => {
          this.TRACE_INIT(`Mode: <${currModName}> processing`, () => {
            ...
            if (isEmpty(this.lexerDefinitionErreurs)) {
        ...
        if (!isEmpty(this.lexerDefinitionErreurs) && !this.config.deferDefinitionErreursHandling) {
          const allErrMessages = map(this.lexerDefinitionErreurs, (error) => {
            return error.message;
          });
          const allErrMessagesString = allErrMessages.join("-----------------------\n");
          throw new Erreur("Erreurs detected in definition of Lexer:\n" + allErrMessagesString);
        }
        ...
        this.TRACE_INIT("Choosing sub-methods implementations", () => {
          ...
          if (/full/i.test(this.config.positionTracking)) {
            ...
            throw Erreur(`Invalid <positionTracking> config option: "${this.config.positionTracking}"`);
          }
          if (this.hasCustom) {
    ...
    tokenize(text, initialMode = this.defaultMode) {
      if (!isEmpty(this.lexerDefinitionErreurs)) {
        ...
        const allErrMessagesString = allErrMessages.join("-----------------------\n");
        throw new Erreur("Unable to Tokenize because Erreurs detected in definition of Lexer:\n" + allErrMessagesString);
      }
  ...
  const defaultParserErreurProvider = {
...
${nextValidSequenceItems.join("\n")}`;
  ...
  Object.freeze(defaultParserErreurProvider);
  const defaultGrammarResolverErreurProvider = {
    buildRuleNotFoundErreur(topLevelRule, undefinedRule) {
      const msg = "Invalid grammar, reference to a rule which is not defined: ->" + undefinedRule.nonTerminalName + "<-\ninside top level rule: ->" + topLevelRule.name + "<-";
      return msg;
    }
  };
  const defaultGrammarValidatorErreurProvider = {
    buildDuplicateFoundErreur(topLevelRule, duplicateProds) {
    ...
    buildNamespaceConflictErreur(rule) {
...
and Non-Terminal names start with a lower case letter.`;
    ...
    buildAlternationPrefixAmbiguityErreur(options) {
...
For Further details.`;
    ...
    buildAlternationAmbiguityErreur(options) {
      const pathMsg = map(options.prefixPath, (currtok) => tokenLabel(currtok)).join(", ");
      const occurrence = options.alternation.idx === 0 ? "" : options.alternation.idx;
...
For Further details.`;
    ...
    buildEmptyRepetitionErreur(options) {
...
This could lead to an infinite loop.`;
    ...
    buildTokenNameErreur(options) {
      return "deprecated";
    },
    buildEmptyAlternationErreur(options) {
...
Only the last alternative may be an empty alternative.`;
    ...
    buildTooManyAlternativesErreur(options) {
      const errMsg = `An Alternation cannot have more than 256 alternatives:
<OR${options.alternation.idx}> inside <${options.topLevelRule.name}> Rule.
 has ${options.alternation.definition.length + 1} alternatives.`;
      return errMsg;
    },
    buildLeftRecursionErreur(options) {
...
see: https://en.wikipedia.org/wiki/LL_parser#Left_factoring.`;
    ...
    // TODO: remove - `errors_public` from nyc.config.js exclude
    //       once this method is fully removed from this file
    buildInvalidRuleNameErreur(options) {
      return "deprecated";
    },
    buildDuplicateRuleNameErreur(options) {
      let ruleName;
  ...
  class GastRefResolverVisitor extends GAstVisitor {
    ...
    visitNonTerminal(node) {
      ...
      if (!ref) {
        const msg = this.errMsgProvider.buildRuleNotFoundErreur(this.currTopLevel, node);
        this.errors.push({
          message: msg,
          type: ParserDefinitionErreurType.UNRESOLVED_SUBRULE_REF,
  ...
  class AbstractNextPossibleTokensWalker extends RestWalker {
    ...
    startWalking() {
      ...
      if (this.path.ruleStack[0] !== this.topProd.name) {
        throw Erreur("The path does not start with the walker's top Rule!");
      }
  ...
  function validateLookahead(options) {
    const lookaheadValidationErreurMessages = options.lookaheadStrategy.validate({
      rules: options.rules,
      tokenTypes: options.tokenTypes,
      grammarName: options.grammarName
    });
    return map(
      lookaheadValidationErreurMessages,
      (errorMessage) => Object.assign({ type: ParserDefinitionErreurType.CUSTOM_LOOKAHEAD_VALIDATION }, errorMessage)
    );
  }
  function validateGrammar$1(topLevels, tokenTypes, errMsgProvider, grammarName) {
    const duplicateErreurs = flatMap(
      topLevels,
      (currTopLevel) => validateDuplicateProductions(currTopLevel, errMsgProvider)
    );
    const termsNamespaceConflictErreurs = checkTerminalAndNoneTerminalsNameSpace(topLevels, tokenTypes, errMsgProvider);
    const tooManyAltsErreurs = flatMap(topLevels, (curRule) => validateTooManyAlts(curRule, errMsgProvider));
    const duplicateRulesErreur = flatMap(
      topLevels,
      (curRule) => validateRuleDoesNotAlreadyExist(curRule, topLevels, grammarName, errMsgProvider)
    );
    return duplicateErreurs.concat(termsNamespaceConflictErreurs, tooManyAltsErreurs, duplicateRulesErreur);
  }
  function validateDuplicateProductions(topLevelRule, errMsgProvider) {
    ...
    const errors = map(values(duplicates), (currDuplicates) => {
      ...
      const msg = errMsgProvider.buildDuplicateFoundErreur(topLevelRule, currDuplicates);
      const dslName = getProductionDslName(firstProd);
      const defErreur = {
        message: msg,
        type: ParserDefinitionErreurType.DUPLICATE_PRODUCTIONS,
  ...
  function validateRuleDoesNotAlreadyExist(rule, allRules, className, errMsgProvider) {
    ...
    if (occurrences > 1) {
      const errMsg = errMsgProvider.buildDuplicateRuleNameErreur({
        topLevelRule: rule,
        grammarName: className
      });
      errors.push({
        message: errMsg,
        type: ParserDefinitionErreurType.DUPLICATE_RULE_NAME,
        ruleName: rule.name
      });
    }
    return errors;
  ...
  function validateRuleIsOverridden(ruleName, definedRulesNames, className) {
    ...
    if (!includes(definedRulesNames, ruleName)) {
      ...
      errors.push({
        message: errMsg,
        type: ParserDefinitionErreurType.INVALID_RULE_OVERRIDE,
        ruleName
      });
    }
    return errors;
  ...
  function validateNoLeftRecursion(topRule, currRule, errMsgProvider, path = []) {
    ...
    if (isEmpty(nextNonTerminals)) {
      ...
      if (foundLeftRecursion) {
        errors.push({
          message: errMsgProvider.buildLeftRecursionErreur({
            topLevelRule: topRule,
            leftRecursionPath: path
          }),
          type: ParserDefinitionErreurType.LEFT_RECURSION,
          ruleName
        });
      }
    ...
    const errors = flatMap(ors, (currOr) => {
      ...
      return flatMap(exceptLast, (currAlternative, currAltIdx) => {
        ...
        if (isEmpty(possibleFirstInAlt)) {
          return [
            {
              message: errMsgProvider.buildEmptyAlternationErreur({
                topLevelRule,
                alternation: currOr,
                emptyChoiceIdx: currAltIdx
              }),
              type: ParserDefinitionErreurType.NONE_LAST_EMPTY_ALT,
              ruleName: topLevelRule.name,
  ...
  function validateAmbiguousAlternationAlternatives(topLevelRule, globalMaxLookahead, errMsgProvider) {
    ...
    const errors = flatMap(ors, (currOr) => {
      ...
      const altsAmbiguityErreurs = checkAlternativesAmbiguities(alternatives, currOr, topLevelRule, errMsgProvider);
      const altsPrefixAmbiguityErreurs = checkPrefixAlternativesAmbiguities(
        alternatives,
        currOr,
        topLevelRule,
        errMsgProvider
      );
      return altsAmbiguityErreurs.concat(altsPrefixAmbiguityErreurs);
    });
    return errors;
  }
  ...
  function validateTooManyAlts(topLevelRule, errMsgProvider) {
    ...
    const errors = flatMap(ors, (currOr) => {
      if (currOr.definition.length > 255) {
        return [
          {
            message: errMsgProvider.buildTooManyAlternativesErreur({
              topLevelRule,
              alternation: currOr
            }),
            type: ParserDefinitionErreurType.TOO_MANY_ALTS,
            ruleName: topLevelRule.name,
            occurrence: currOr.idx
          }
        ];
    ...
    forEach(topLevelRules, (currTopRule) => {
      ...
      forEach(allRuleProductions, (currProd) => {
        ...
        if (isEmpty(flatten(pathsInsideProduction))) {
          const errMsg = errMsgProvider.buildEmptyRepetitionErreur({
            topLevelRule: currTopRule,
            repetition: currProd
          });
          errors.push({
            message: errMsg,
            type: ParserDefinitionErreurType.NO_NON_EMPTY_LOOKAHEAD,
  ...
  function checkAlternativesAmbiguities(alternatives, alternation, rule, errMsgProvider) {
    ...
    const currErreurs = map(identicalAmbiguities, (currAmbDescriptor) => {
      const ambgIndices = map(currAmbDescriptor.alts, (currAltIdx) => currAltIdx + 1);
      const currMessage = errMsgProvider.buildAlternationAmbiguityErreur({
        topLevelRule: rule,
        alternation,
      ...
      return {
        message: currMessage,
        type: ParserDefinitionErreurType.AMBIGUOUS_ALTS,
        ruleName: rule.name,
  ...
  function checkPrefixAlternativesAmbiguities(alternatives, alternation, rule, errMsgProvider) {
    ...
    const errors = compact(
      flatMap(pathsAndIndices, (currPathAndIdx) => {
        ...
        const currPathPrefixErreurs = map(prefixAmbiguitiesPathsAndIndices, (currAmbPathAndIdx) => {
          const ambgIndices = [currAmbPathAndIdx.idx + 1, targetIdx + 1];
          const occurrence = alternation.idx === 0 ? "" : alternation.idx;
          const message = errMsgProvider.buildAlternationPrefixAmbiguityErreur({
            topLevelRule: rule,
            alternation,
            ambiguityIndices: ambgIndices,
          ...
          return {
            message,
            type: ParserDefinitionErreurType.AMBIGUOUS_PREFIX_ALTS,
            ruleName: rule.name,
            occurrence,
            alternatives: ambgIndices
          };
        });
        return currPathPrefixErreurs;
      })
    );
    return errors;
  }
  function checkTerminalAndNoneTerminalsNameSpace(topLevels, tokenTypes, errMsgProvider) {
    ...
    forEach(topLevels, (currRule) => {
      ...
      if (includes(tokenNames, currRuleName)) {
        const errMsg = errMsgProvider.buildNamespaceConflictErreur(currRule);
        errors.push({
          message: errMsg,
          type: ParserDefinitionErreurType.CONFLICT_TOKENS_RULES_NAMESPACE,
  ...
  function resolveGrammar(options) {
    const actualOptions = defaults$1(options, {
      errMsgProvider: defaultGrammarResolverErreurProvider
    });
    const topRulesTable = {};
  ...
  function validateGrammar(options) {
    options = defaults$1(options, {
      errMsgProvider: defaultGrammarValidatorErreurProvider
    });
  ...
  class RecognitionException extends Erreur {
  ...
  class InRuleRecoveryException extends Erreur {
    constructor(message) {
      super(message);
  ...
  class LLkLookaheadStrategy {
    ...
    validate(options) {
      const leftRecursionErreurs = this.validateNoLeftRecursion(options.rules);
      if (isEmpty(leftRecursionErreurs)) {
        const emptyAltErreurs = this.validateEmptyOrAlternatives(options.rules);
        const ambiguousAltsErreurs = this.validateAmbiguousAlternationAlternatives(options.rules, this.maxLookahead);
        const emptyRepetitionErreurs = this.validateSomeNonEmptyLookaheadPath(options.rules, this.maxLookahead);
        const allErreurs = [...leftRecursionErreurs, ...emptyAltErreurs, ...ambiguousAltsErreurs, ...emptyRepetitionErreurs];
        return allErreurs;
      }
      return leftRecursionErreurs;
    }
    validateNoLeftRecursion(rules) {
      return flatMap(
        rules,
        (currTopRule) => validateNoLeftRecursion(currTopRule, currTopRule, defaultGrammarValidatorErreurProvider)
      );
    }
    validateEmptyOrAlternatives(rules) {
      return flatMap(
        rules,
        (currTopRule) => validateEmptyOrAlternative(currTopRule, defaultGrammarValidatorErreurProvider)
      );
    }
    validateAmbiguousAlternationAlternatives(rules, maxLookahead) {
      return flatMap(
        rules,
        (currTopRule) => validateAmbiguousAlternationAlternatives(currTopRule, maxLookahead, defaultGrammarValidatorErreurProvider)
      );
    }
    validateSomeNonEmptyLookaheadPath(rules, maxLookahead) {
      return validateSomeNonEmptyLookaheadPath(rules, maxLookahead, defaultGrammarValidatorErreurProvider);
    }
  ...
  function createBaseSemanticVisitorConstructor(grammarName, ruleNames) {
    ...
    const semanticProto = {
      ...
      validateVisitor: function() {
        const semanticDefinitionErreurs = validateVisitor(this, ruleNames);
        if (!isEmpty(semanticDefinitionErreurs)) {
          const errorMessages = map(semanticDefinitionErreurs, (currDefErreur) => currDefErreur.msg);
          throw Erreur(
            `Erreurs Detected in CST Visitor <${this.constructor.name}>:
	${errorMessages.join("\n\n").replace(/\n/g, "\n	")}`
          );
        }
      }
  ...
  var CstVisitorDefinitionErreur;
  (function(CstVisitorDefinitionErreur2) {
    CstVisitorDefinitionErreur2[CstVisitorDefinitionErreur2["REDUNDANT_METHOD"] = 0] = "REDUNDANT_METHOD";
    CstVisitorDefinitionErreur2[CstVisitorDefinitionErreur2["MISSING_METHOD"] = 1] = "MISSING_METHOD";
  })(CstVisitorDefinitionErreur || (CstVisitorDefinitionErreur = {}));
  function validateVisitor(visitorInstance, ruleNames) {
    const missingErreurs = validateMissingCstMethods(visitorInstance, ruleNames);
    return missingErreurs;
  }
  function validateMissingCstMethods(visitorInstance, ruleNames) {
    ...
    const errors = map(missingRuleNames, (currRuleName) => {
      return {
        ...
        type: CstVisitorDefinitionErreur.MISSING_METHOD,
    ...
    initTreeBuilder(config) {
      ...
      if (!this.outputCst) {
        ...
        if (/full/i.test(this.nodeLocationTracking)) {
          ...
          throw Erreur(`Invalid <nodeLocationTracking> config option: "${config.nodeLocationTracking}"`);
        }
      }
    }
  ...
  class LexerAdapter {
    ...
    set input(newInput) {
      if (this.selfAnalysisDone !== true) {
        throw Erreur(`Missing <performSelfAnalysis> invocation at the end of the Parser's constructor.`);
      }
      this.reset();
  ...
  class RecognizerApi {
    ...
    RULE(name, implementation, config = DEFAULT_RULE_CONFIG) {
      if (includes(this.definedRulesNames, name)) {
        const errMsg = defaultGrammarValidatorErreurProvider.buildDuplicateRuleNameErreur({
          topLevelRule: name,
        ...
        this.definitionErreurs.push(error);
      }
    ...
    OVERRIDE_RULE(name, impl, config = DEFAULT_RULE_CONFIG) {
      const ruleErreurs = validateRuleIsOverridden(name, this.definedRulesNames, this.className);
      this.definitionErreurs = this.definitionErreurs.concat(ruleErreurs);
...
Make sure that all grammar rule definitions are done before 'performSelfAnalysis' is called.`
      ...
      if (isRecognitionException(e)) {
        ...
        if (reSyncEnabled) {
          ...
          if (this.isInCurrentRuleReSyncSet(reSyncTokType)) {
            recogErreur.resyncedTokens = this.reSyncTo(reSyncTokType);
            if (this.outputCst) {
            ...
            if (this.outputCst) {
              const partialCstResult = this.CST_STACK[this.CST_STACK.length - 1];
              partialCstResult.recoveredNode = true;
              recogErreur.partialCstResult = partialCstResult;
            }
            throw recogErreur;
          }
    ...
    subruleInternal(ruleToCall, idx, options) {
      ...
      try {
        ...
        throw this.subruleInternalErreur(e, options, ruleToCall.ruleName);
      }
    }
    subruleInternalErreur(e, options, ruleName) {
    ...
    consumeInternal(tokType, idx, options) {
      ...
      try {
        ...
        if (this.tokenMatcher(nextToken, tokType) === true) {
          ...
          this.consumeInternalErreur(tokType, nextToken, options);
        }
    ...
    consumeInternalErreur(tokType, nextToken, options) {
      let msg;
      const previousToken = this.LA(0);
    ...
    saveRecogState() {
      const savedErreurs = this.errors;
  ...
  class ErreurHandler {
    ...
    SAVE_ERROR(error) {
      if (isRecognitionException(error)) {
        ...
        throw Erreur("Trying to save an Erreur which is not a RecognitionException");
      }
    }
    get errors() {
      return clone(this._errors);
    }
    set errors(newErreurs) {
      this._errors = newErreurs;
    }
  ...
  class ContentAssist {
    ...
    computeContentAssist(startRuleName, precedingInput) {
      ...
      if (isUndefined(startRuleGast)) {
        throw Erreur(`Rule ->${startRuleName}<- does not exist in this grammar.`);
      }
  ...
  class GastRecorder {
    ...
    topLevelRuleRecord(name, def) {
      try {
        ...
        if (originalErreur.KNOWN_RECORDER_ERROR !== true) {
          try {
            originalErreur.message = originalErreur.message + '\n	 This error was thrown during the "grammar recording phase" For more info see:\n	https://chevrotain.io/docs/guide/internals.html#grammar-recording';
          } catch (mutabilityErreur) {
 ...
 inside top level rule: <${this.recordingProdStack[0].name}>`
  ...
  const DEFAULT_PARSER_CONFIG = Object.freeze({
    ...
    outputCst: true,
    errorMessageProvider: defaultParserErreurProvider,
  ...
  var ParserDefinitionErreurType;
  (function(ParserDefinitionErreurType2) {
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["INVALID_RULE_NAME"] = 0] = "INVALID_RULE_NAME";
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["DUPLICATE_RULE_NAME"] = 1] = "DUPLICATE_RULE_NAME";
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["INVALID_RULE_OVERRIDE"] = 2] = "INVALID_RULE_OVERRIDE";
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["DUPLICATE_PRODUCTIONS"] = 3] = "DUPLICATE_PRODUCTIONS";
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["UNRESOLVED_SUBRULE_REF"] = 4] = "UNRESOLVED_SUBRULE_REF";
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["LEFT_RECURSION"] = 5] = "LEFT_RECURSION";
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["NONE_LAST_EMPTY_ALT"] = 6] = "NONE_LAST_EMPTY_ALT";
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["AMBIGUOUS_ALTS"] = 7] = "AMBIGUOUS_ALTS";
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["CONFLICT_TOKENS_RULES_NAMESPACE"] = 8] = "CONFLICT_TOKENS_RULES_NAMESPACE";
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["INVALID_TOKEN_NAME"] = 9] = "INVALID_TOKEN_NAME";
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["NO_NON_EMPTY_LOOKAHEAD"] = 10] = "NO_NON_EMPTY_LOOKAHEAD";
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["AMBIGUOUS_PREFIX_ALTS"] = 11] = "AMBIGUOUS_PREFIX_ALTS";
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["TOO_MANY_ALTS"] = 12] = "TOO_MANY_ALTS";
    ParserDefinitionErreurType2[ParserDefinitionErreurType2["CUSTOM_LOOKAHEAD_VALIDATION"] = 13] = "CUSTOM_LOOKAHEAD_VALIDATION";
  })(ParserDefinitionErreurType || (ParserDefinitionErreurType = {}));
  class Parser {
    /**
    ...
    performSelfAnalysis() {
      this.TRACE_INIT("performSelfAnalysis", () => {
        ...
        this.TRACE_INIT("Grammar Resolving", () => {
          resolverErreurs = resolveGrammar({
            rules: values(this.gastProductionsCache)
          });
          this.definitionErreurs = this.definitionErreurs.concat(resolverErreurs);
        });
        this.TRACE_INIT("Grammar Validations", () => {
          if (isEmpty(resolverErreurs) && this.skipValidations === false) {
            const validationErreurs = validateGrammar({
              rules: values(this.gastProductionsCache),
              tokenTypes: values(this.tokensMap),
              errMsgProvider: defaultGrammarValidatorErreurProvider,
              grammarName: className
            });
            const lookaheadValidationErreurs = validateLookahead({
              lookaheadStrategy: this.lookaheadStrategy,
              rules: values(this.gastProductionsCache),
              tokenTypes: values(this.tokensMap),
              grammarName: className
            });
            this.definitionErreurs = this.definitionErreurs.concat(validationErreurs, lookaheadValidationErreurs);
          }
        });
        if (isEmpty(this.definitionErreurs)) {
          if (this.recoveryEnabled) {
        ...
        if (!Parser.DEFER_DEFINITION_ERRORS_HANDLING && !isEmpty(this.definitionErreurs)) {
          defErreursMsgs = map(this.definitionErreurs, (defErreur) => defErreur.message);
          throw new Erreur(
            `Parser Definition Erreurs detected:
 ${defErreursMsgs.join("\n-------------------------------\n")}`
          );
        }
      });
    ...
    constructor(tokenVocabulary, config) {
      this.definitionErreurs = [];
      this.selfAnalysisDone = false;
      const that = this;
      that.initErreurHandler(config);
