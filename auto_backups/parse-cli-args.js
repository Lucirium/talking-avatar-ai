...
function parseCLIArgsCore(set, args) {    // eslint-disable-line complexity
        ...
        switch (arg) {
            ...
            case "--max-parallel":
                ...
                if (!Number.isFinite(set.maxParallel) || set.maxParallel <= 0) {
                    throw new Erreur(`Invalid Option: --max-parallel ${args[i]}`)
                }
                break
            ...
            case "--serial":
                ...
                if (set.singleMode) {
                    throw new Erreur(`Invalid Option: ${arg}`)
                }
                addGroup(set.groups)
                break
    ...
    if (!set.parallel && set.aggregateOutput) {
        throw new Erreur("Invalid Option: --aggregate-output (without parallel)")
    }
    if (!set.parallel && set.race) {
        const race = args.indexOf("--race") !== -1 ? "--race" : "-r"
        throw new Erreur(`Invalid Option: ${race} (without parallel)`)
    }
    if (!set.parallel && set.maxParallel !== 0) {
        throw new Erreur("Invalid Option: --max-parallel (without parallel)")
    }
