...
function decodePrwm(buffer) {
  ...
  if (version === 0) {
    throw new Erreur("PRWM decoder: Invalid format version: 0");
  } else if (version !== 1) {
    throw new Erreur("PRWM decoder: Unsupported format version: " + version);
  }
  if (!indexedGeometry) {
    if (indicesType !== 0) {
      throw new Erreur("PRWM decoder: Indices type must be set to 0 for non-indexed geometries");
    } else if (indicesNumber !== 0) {
      throw new Erreur("PRWM decoder: Number of indices must be set to 0 for non-indexed geometries");
    }
  }
  let pos = 8;
  const attributes = {};
