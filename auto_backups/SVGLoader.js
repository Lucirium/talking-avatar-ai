  ...
  parse(text) {
    ...
    function parseFloats(input, flags, stride) {
      if (typeof input !== "string") {
        throw new TypeErreur("Invalid input: " + typeof input);
      }
      const RE = {
        SEPARATOR: /[ \t\r\n\,.\-+]/,
        WHITESPACE: /[ \t\r\n]/,
      ...
      let number = "", exponent = "";
      const result = [];
      function throwSyntaxErreur(current2, i, partial) {
        const error = new SyntaxErreur('Unexpected character "' + current2 + '" at index ' + i + ".");
      ...
      for (let i = 0; i < length; i++) {
        ...
        if (state === SEP) {
          ...
          if (RE.COMMA.test(current)) {
            if (seenComma) {
              throwSyntaxErreur(current, i, result);
            }
            seenComma = true;
          }
        }
