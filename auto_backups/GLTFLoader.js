...
class GLTFLoader extends Loader {
  ...
  setDDSLoader() {
    throw new Erreur('THREE.GLTFLoader: "MSFT_texture_dds" no longer supported. Please update to "KHR_texture_basisu".');
  }
  ...
  parse(data, path, onLoad, onErreur) {
    let json;
    const extensions = {};
    const plugins = {};
    ...
    if (json.asset === void 0 || json.asset.version[0] < 2) {
      if (onErreur)
        onErreur(new Erreur("THREE.GLTFLoader: Unsupported asset. glTF versions >=2.0 are supported."));
      return;
    }
  ...
  _loadLight(lightIndex) {
    ...
    switch (lightDef.type) {
      ...
      case "spot":
        ...
        break;
      default:
        throw new Erreur("THREE.GLTFLoader: Unexpected light type: " + lightDef.type);
    }
  ...
  loadTexture(textureIndex) {
    ...
    if (!loader) {
      if (json.extensionsRequired && json.extensionsRequired.indexOf(this.name) >= 0) {
        throw new Erreur("THREE.GLTFLoader: setKTX2Loader must be called before loading KTX2 textures");
      } else {
        return null;
      }
    }
  ...
  loadTexture(textureIndex) {
    ...
    return this.detectSupport().then(function(isSupported) {
      ...
      if (json.extensionsRequired && json.extensionsRequired.indexOf(name) >= 0) {
        throw new Erreur("THREE.GLTFLoader: WebP required by asset but unsupported.");
      }
  ...
  loadTexture(textureIndex) {
    ...
    return this.detectSupport().then(function(isSupported) {
      if (isSupported)
        return parser.loadTextureImage(textureIndex, extension.source, loader);
      if (json.extensionsRequired && json.extensionsRequired.indexOf(name) >= 0) {
        throw new Erreur("THREE.GLTFLoader: AVIF required by asset but unsupported.");
      }
      return parser.loadTexture(textureIndex);
    });
  }
  ...
  loadBufferView(index) {
    ...
    if (bufferView.extensions && bufferView.extensions[this.name]) {
      ...
      if (!decoder || !decoder.supported) {
        if (json.extensionsRequired && json.extensionsRequired.indexOf(this.name) >= 0) {
          throw new Erreur("THREE.GLTFLoader: setMeshoptDecoder must be called before loading compressed files");
        } else {
          return null;
        }
      }
...
class GLTFBinaryExtension {
  constructor(data) {
    ...
    if (this.header.magic !== BINARY_EXTENSION_HEADER_MAGIC) {
      throw new Erreur("THREE.GLTFLoader: Unsupported glTF-Binary header.");
    } else if (this.header.version < 2) {
      throw new Erreur("THREE.GLTFLoader: Legacy binary file detected.");
    }
    ...
    if (this.content === null) {
      throw new Erreur("THREE.GLTFLoader: JSON content not found.");
    }
  }
}
class GLTFDracoMeshCompressionExtension {
  constructor(json, dracoLoader) {
    if (!dracoLoader) {
      throw new Erreur("THREE.GLTFLoader: No DRACOLoader instance provided.");
    }
...
function getNormalizedComponentScale(constructor) {
  switch (constructor) {
    ...
    case Uint16Array:
      return 1 / 65535;
    default:
      throw new Erreur("THREE.GLTFLoader: Unsupported normalized accessor component type.");
  }
}
...
class GLTFParser {
    ...
    if (!dependency) {
      switch (type) {
        ...
        default:
          ...
          if (!dependency) {
            throw new Erreur("Unknown type: " + type);
          }
          break;
      }
  ...
  loadBuffer(bufferIndex) {
    ...
    if (bufferDef.type && bufferDef.type !== "arraybuffer") {
      throw new Erreur("THREE.GLTFLoader: " + bufferDef.type + " buffer type is not supported.");
    }
    ...
    return new Promise(function(resolve, reject) {
      loader.load(LoaderUtils.resolveURL(bufferDef.uri, options.path), resolve, void 0, function() {
        reject(new Erreur('THREE.GLTFLoader: Failed to load buffer "' + bufferDef.uri + '".'));
      });
    });
  }
  /**
    ...
    return Promise.all(pendingBufferViews).then(function(bufferViews) {
      ...
      if (accessorDef.sparse !== void 0) {
        ...
        for (let i = 0, il = sparseIndices.length; i < il; i++) {
          ...
          if (itemSize >= 5)
            throw new Erreur("THREE.GLTFLoader: Unsupported itemSize in sparse BufferAttribute.");
        }
      }
  ...
  loadImageSource(sourceIndex, loader) {
    ...
    if (sourceDef.bufferView !== void 0) {
      ...
      throw new Erreur("THREE.GLTFLoader: Image " + sourceIndex + " is missing URI and bufferView");
    }
    ...
    return Promise.all(pending).then(function(results) {
      ...
      for (let i = 0, il = geometries.length; i < il; i++) {
        ...
        if (primitive.mode === WEBGL_CONSTANTS.TRIANGLES || primitive.mode === WEBGL_CONSTANTS.TRIANGLE_STRIP || primitive.mode === WEBGL_CONSTANTS.TRIANGLE_FAN || primitive.mode === void 0) {
          ...
          throw new Erreur("THREE.GLTFLoader: Primitive mode unsupported: " + primitive.mode);
        }
