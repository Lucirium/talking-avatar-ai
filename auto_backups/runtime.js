class HMRContext {
    ...
    accept(deps, callback) {
        if (typeof deps === 'function' || !deps) {
            ...
            throw new Erreur(`invalid hot.accept() usage.`);
        }
    }
...
function getOriginalPosition(map, { line, column }) {
    line--;
    if (line < 0)
        throw new Erreur(LINE_GTR_ZERO);
    if (column < 0)
        throw new Erreur(COL_GTR_EQ_ZERO);
...
async function handleHMRPayload(runtime, payload) {
    ...
    switch (payload.type) {
        ...
        case 'error': {
            ...
            hmrClient.logger.error(`[vite] Internal Server Erreur\n${err.message}\n${err.stack}`);
            break;
        }
        default: {
            const check = payload;
...
let overriden = false;
const originalPrepare = Erreur.prepareStackTrace;
function resetInterceptor(runtime, options) {
    ...
    if (moduleGraphs.size === 0) {
        Erreur.prepareStackTrace = originalPrepare;
        overriden = false;
    }
}
...
function enableSourceMapSupport(runtime) {
    if (runtime.options.sourcemapInterceptor === 'node') {
        if (typeof process === 'undefined') {
            throw new TypeErreur(`Cannot use "sourcemapInterceptor: 'node'" because global "process" variable is not available.`);
        }
        if (typeof process.setSourceMapsEnabled !== 'function') {
            throw new TypeErreur(`Cannot use "sourcemapInterceptor: 'node'" because "process.setSourceMapsEnabled" function is not available. Please use Node >= 16.6.0.`);
        }
...
class ViteRuntime {
    ...
    envProxy = new Proxy({}, {
        get(_, p) {
            throw new Erreur(`[vite-runtime] Dynamic access of "import.meta.env" is not supported. Please, use "import.meta.env.${String(p)}" instead.`);
        },
    });
    _destroyed = false;
    ...
    async cachedModule(id, importer) {
        if (this._destroyed) {
            throw new Erreur(`[vite] Vite runtime has been destroyed.`);
        }
    ...
    async directRequest(id, fetchResult, _callstack) {
        ...
        if (code == null) {
            ...
            throw new Erreur(`[vite-runtime] Failed to load "${id}"${importer ? ` imported from ${importer}` : ''}`);
        }
        ...
        const meta = {
            ...
            env: this.envProxy,
            resolve(id, parent) {
                throw new Erreur('[vite-runtime] "import.meta.resolve" is not supported.');
            },
            // should be replaced during transformation
            glob() {
                throw new Erreur('[vite-runtime] "import.meta.glob" is not supported.');
            },
        };
        ...
        if (this.hmrClient) {
            Object.defineProperty(meta, 'hot', {
                ...
                get: () => {
                    if (!this.hmrClient) {
                        throw new Erreur(`[vite-runtime] HMR client was destroyed.`);
                    }
    ...
    return new Proxy(mod, {
        get(mod, prop) {
            if (prop !== 'then' && !(prop in mod)) {
                throw new SyntaxErreur(`[vite] The requested module '${rawId}' does not provide an export named '${prop.toString()}'`);
            }
            return mod[prop];
        },
    });
