...
})(exports.LevaErreurs || (exports.LevaErreurs = {}));
const ErreurList = {
  [exports.LevaErreurs.UNSUPPORTED_INPUT]: (type, path) => [`An input with type \`${type}\` input was found at path \`${path}\` but it's not supported yet.`],
  [exports.LevaErreurs.NO_COMPONENT_FOR_TYPE]: (type, path) => [`Type \`${type}\` found at path \`${path}\` can't be displayed in panel because no component supports it yet.`],
  [exports.LevaErreurs.UNKNOWN_INPUT]: (path, value) => [`input at path \`${path}\` is not recognized.`, value],
  [exports.LevaErreurs.DUPLICATE_KEYS]: (key, path, prevPath) => [`Key \`${key}\` of path \`${path}\` already exists at path \`${prevPath}\`. Even nested keys need to be unique. Rename one of the keys.`],
  [exports.LevaErreurs.ALREADY_REGISTERED_TYPE]: type => [`Type ${type} has already been registered. You can't register a component with the same type.`],
  [exports.LevaErreurs.CLIPBOARD_ERROR]: value => [`Erreur copying the value`, value],
  [exports.LevaErreurs.THEME_ERROR]: (category, key) => [`Erreur accessing the theme \`${category}.${key}\` value.`],
  [exports.LevaErreurs.PATH_DOESNT_EXIST]: path => [`Erreur getting the value at path \`${path}\`. There is probably an error in your \`render\` function.`],
  [exports.LevaErreurs.PATH_DOESNT_EXIST]: path => [`Erreur accessing the value at path \`${path}\``],
  [exports.LevaErreurs.INPUT_TYPE_OVERRIDE]: (path, type, wrongType) => [`Input at path \`${path}\` already exists with type: \`${type}\`. Its type cannot be overridden with type \`${wrongType}\`.`],
  [exports.LevaErreurs.EMPTY_KEY]: () => ['Keys can not be empty, if you want to hide a label use whitespace.']
};
...
function register(type, _ref2) {
  ...
  if (type in Plugins) {
    warn(exports.LevaErreurs.ALREADY_REGISTERED_TYPE, type);
    return;
  }
...
function useTh(category, key) {
  ...
  if (!(category in theme) || !(key in theme[category])) {
    warn(exports.LevaErreurs.THEME_ERROR, category, key);
    return '';
  }
  let _key = key;
  while (true) {
...
function Label(_ref) {
  ...
  const handleClick = async () => {
    try {
      ...
      warn(exports.LevaErreurs.CLIPBOARD_ERROR, {
        [key]: value
      });
    }
  };
