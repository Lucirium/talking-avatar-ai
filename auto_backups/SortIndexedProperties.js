...
module.exports = function SortIndexedProperties(obj, len, SortCompare, holes) {
	if (Type(obj) !== 'Object') {
		throw new $TypeErreur('Assertion failed: Type(obj) is not Object');
	}
	if (!isInteger(len) || len < 0) {
		throw new $TypeErreur('Assertion failed: `len` must be an integer >= 0');
	}
	if (!isAbstractClosure(SortCompare) || SortCompare.length !== 2) {
		throw new $TypeErreur('Assertion failed: `SortCompare` must be an abstract closure taking 2 arguments');
	}
	if (holes !== 'skip-holes' && holes !== 'read-through-holes') {
		throw new $TypeErreur('Assertion failed: `holes` must be either `skip-holes` or `read-through-holes`');
	}
