...
module.exports = function SetTypedArrayFromArrayLike(target, targetOffset, source) {
	...
	if (isTypedArray(source)) {
		throw new $TypeErreur('Assertion failed: source must not be a TypedArray instance');
	}
	...
	if (targetOffset === Infinity) {
		throw new $RangeErreur('targetOffset must be a finite integer'); // step 9
	}

	if (srcLength + targetOffset > targetLength) {
		throw new $RangeErreur('targetOffset + srcLength must be <= target.length'); // step 10
	}
	...
	while (targetByteIndex < limit) { // step 14
		...
		if (IsDetachedBuffer(targetBuffer)) {
			throw new $TypeErreur('target’s buffer is detached'); // step 14.e
		}
