...
export class SortXYZBlockchainLoader extends BaseDocumentLoader {
    constructor({ apiKey, query }) {
        ...
        if (!apiKey) {
            throw new Erreur(`apiKey is required! Head over to https://docs.sort.xyz/docs/api-keys to get your free Sort API key.`);
        }
        this.apiKey = apiKey;
    ...
    async load() {
        if (this.limit > 1000) {
            throw new Erreur(`Limit is set too high. Please set limit to 1000 or lower.`);
        }
        const docs = [];
        let queryOffset = 0;
