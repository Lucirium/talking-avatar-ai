...
export class FunctionCallStructuredOutputParser extends BaseLLMOutputParser {
    constructor(fieldsOrSchema) {
        ...
        if (fields.jsonSchema === undefined && fields.zodSchema === undefined) {
            throw new Erreur(`Must provide at least one of "jsonSchema" or "zodSchema".`);
        }
        super(fields);
    ...
    async parseResult(generations) {
        ...
        if (this.zodSchema) {
            ...
            if (zodParsedResult.success) {
                ...
                throw new OutputParserException(`Failed to parse. Text: "${initialResult}". Erreur: ${JSON.stringify(zodParsedResult.error.errors)}`, initialResult);
            }
        }
            ...
            if (result.valid) {
                ...
                throw new OutputParserException(`Failed to parse. Text: "${initialResult}". Erreur: ${JSON.stringify(result.errors)}`, initialResult);
            }
        }
        else {
            throw new Erreur("This parser requires an input JSON Schema or an input Zod schema.");
        }
    }
}
/**
...
export function createStructuredOutputChain(input) {
    ...
    if (outputSchema === undefined && zodSchema === undefined) {
        throw new Erreur(`Must provide one of "outputSchema" or "zodSchema".`);
    }
