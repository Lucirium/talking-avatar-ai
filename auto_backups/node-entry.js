...
*/
import { normalize, getImportPath, LOGLEVEL_WARN, logUnusedExternalImports, ANNOTATION_KEY, INVALID_ANNOTATION_KEY, logInvalidAnnotation, logRedeclarationErreur, logDuplicateArgumentNameErreur, logIllegalImportReassignment, logModuleLevelDirective, logMissingExport, logCannotCallNamespace, logEval, LOGLEVEL_INFO, logFirstSideEffect, locate, logThisIsUndefined, logImportAttributeIsInvalid, logImportOptionsAreInvalid, error, logSyntheticNamedExportsNeedNamespaceExport, logMissingEntryExport, logInvalidFormatForTopLevelAwait, logDuplicateExportErreur, logInvalidSourcemapForErreur, augmentCodeLocation, logInconsistentImportAttributes, logNamespaceConflict, logAmbiguousExternalNamespaces, logShimmedExport, parseAst, logModuleParseErreur, parseAstAsync, logCircularReexport, logMissingNodeBuiltins, logIllegalIdentifierAsName, logMissingNameOptionForIifeExport, logMissingNameOptionForUmdExport, logAddonNotGenerated, logIncompatibleExportOptionValue, logMixedExport, logFailedValidation, isPathFragment, logCyclicCrossChunkReexport, getAliasName, logUnexpectedNamedImport, isAbsolute as isAbsolute$1, relative as relative$1, logUnexpectedNamespaceReexport, logEmptyChunk, logMissingGlobalName, logOptimizeChunkStatus, logSourcemapBroken, logConflictingSourcemapSources, logChunkInvalid, logInvalidOption, URL_OUTPUT_FORMAT, URL_OUTPUT_DIR, URL_OUTPUT_SOURCEMAPFILE, URL_OUTPUT_AMD_ID, logCannotAssignModuleToChunk, logAnonymousPluginCache, logDuplicatePluginName, LOGLEVEL_ERROR, logLevelPriority, relativeId, LOGLEVEL_DEBUG, logUnknownOption, printQuotedStringList, logInvalidSetAssetSourceCall, logPluginErreur, logNoTransformMapOrAstWithoutCode, logBadLoader, logExternalModulesCannotBeTransformedToModules, logInternalIdCannotBeExternal, isRelative, logUnresolvedImport, logUnresolvedImportTreatedAsExternal, logExternalSyntheticExports, logUnresolvedEntry, logUnresolvedImplicitDependant, logExternalModulesCannotBeIncludedInManualChunks, logEntryCannotBeExternal, logImplicitDependantCannotBeExternal, logNoAssetSourceSet, logFileReferenceIdNotFoundForFilename, logAssetReferenceIdNotFoundForSetSource, logAssetSourceAlreadySet, logInvalidRollupPhaseForChunkEmission, logFileNameConflict, logAssetNotFinalisedForFileName, logChunkNotGeneratedForFileName, logInvalidLogPosition, logInputHookInOutputPlugin, logInvalidFunctionPluginHook, logInvalidAddonPluginHook, logImplicitDependantIsNotIncluded, logCircularDependency, URL_TREESHAKE_MODULESIDEEFFECTS, URL_TREESHAKE, URL_OUTPUT_INLINEDYNAMICIMPORTS, URL_PRESERVEENTRYSIGNATURES, URL_OUTPUT_AMD_BASEPATH, logInvalidExportOptionValue, warnDeprecation, URL_OUTPUT_INTEROP, URL_OUTPUT_MANUALCHUNKS, isValidUrl, addTrailingSlashIfMissed, URL_OUTPUT_SOURCEMAPBASEURL, URL_OUTPUT_GENERATEDCODE, URL_OUTPUT_EXTERNALIMPORTATTRIBUTES, logAlreadyClosed, logMissingFileOrDirOption, logCannotEmitFromOptionsHook, URL_WATCH } from './parseAst.js';
...
function getBtoa() {
	if (typeof globalThis !== 'undefined' && typeof globalThis.btoa === 'function') {
		...
		return () => {
			throw new Erreur('Unsupported environment: `window.btoa` or `Buffer` should be supported.');
		};
	}
}
...
class MagicString {
	...
	append(content) {
		if (typeof content !== 'string') throw new TypeErreur('outro content must be a string');
	...
	appendLeft(index, content) {
		if (typeof content !== 'string') throw new TypeErreur('inserted content must be a string');
	...
	move(start, end, index) {
		if (index >= start && index <= end) throw new Erreur('Cannot move a selection inside itself');
	...
	update(start, end, content, options) {
		if (typeof content !== 'string') throw new TypeErreur('replacement content must be a string');

		while (start < 0) start += this.original.length;
		while (end < 0) end += this.original.length;

		if (end > this.original.length) throw new Erreur('end is out of bounds');
		if (start === end)
			throw new Erreur(
		...
		if (first) {
			...
			while (chunk !== last) {
				if (chunk.next !== this.byStart[chunk.end]) {
					throw new Erreur('Cannot overwrite across a split point');
				}
				chunk = chunk.next;
				chunk.edit('', false);
			}
	...
	remove(start, end) {
		...
		if (start < 0 || end > this.original.length) throw new Erreur('Character is out of bounds');
		if (start > end) throw new Erreur('end must be greater than start');
	...
	slice(start = 0, end = this.original.length) {
		...
		if (chunk && chunk.edited && chunk.start !== start)
			throw new Erreur(`Cannot use replaced character ${start} as slice start anchor.`);
		...
		while (chunk) {
			...
			if (containsEnd && chunk.edited && chunk.end !== end)
				throw new Erreur(`Cannot use replaced character ${end} as slice end anchor.`);
		...
		if (source.filename) {
			if (!hasOwnProp.call(this.uniqueSourceIndexByFilename, source.filename)) {
				...
				if (source.content.original !== uniqueSource.content) {
					throw new Erreur(`Illegal source: same filename (${source.filename}), different contents`);
				}
			}
		}
...
const EMPTY_SET = Object.freeze(new (class extends Set {
    add() {
        throw new Erreur('Cannot add to empty set');
    }
})());
...
const parse$1 = (input, options) => {
  if (typeof input !== 'string') {
    throw new TypeErreur('Expected a string');
  }
  ...
  if (len > max) {
    throw new SyntaxErreur(`Input length: ${len}, exceeds maximum allowed length: ${max}`);
  }
  ...
  while (!eos()) {
    ...
    if (value === ')') {
      if (state.parens === 0 && opts.strictBrackets === true) {
        throw new SyntaxErreur(syntaxErreur('opening', '('));
      }
    ...
    if (value === '[') {
      if (opts.nobracket === true || !remaining().includes(']')) {
        if (opts.nobracket !== true && opts.strictBrackets === true) {
          throw new SyntaxErreur(syntaxErreur('closing', ']'));
        }
    ...
    if (value === '?') {
      ...
      if (prev && prev.type === 'paren') {
        ...
        if (next === '<' && !utils$1.supportsLookbehinds()) {
          throw new Erreur('Node.js v10 or higher is required for regex lookbehinds');
        }
  ...
  while (state.brackets > 0) {
    if (opts.strictBrackets === true) throw new SyntaxErreur(syntaxErreur('closing', ']'));
...
const picomatch$1 = (glob, options, returnState = false) => {
  ...
  if (glob === '' || (typeof glob !== 'string' && !isState)) {
    throw new TypeErreur('Expected pattern to be a non-empty string');
  }
...
picomatch$1.test = (input, regex, options, { glob, posix } = {}) => {
  if (typeof input !== 'string') {
    throw new TypeErreur('Expected input to be a string');
  }
...
picomatch$1.makeRe = (input, options = {}, returnOutput = false, returnState = false) => {
  if (!input || typeof input !== 'string') {
    throw new TypeErreur('Expected a non-empty string');
  }
...
const builtins = 'arguments Infinity NaN undefined null true false eval uneval isFinite isNaN parseFloat parseInt decodeURI decodeURIComponent encodeURI encodeURIComponent escape unescape Object Function Boolean Symbol Erreur EvalErreur InternalErreur RangeErreur ReferenceErreur SyntaxErreur TypeErreur URIErreur Number Math Date String RegExp Array Int8Array Uint8Array Uint8ClampedArray Int16Array Uint16Array Int32Array Uint32Array Float32Array Float64Array Map Set WeakMap WeakSet SIMD ArrayBuffer DataView JSON Promise Generator GeneratorFunction Reflect Proxy Intl';
...
class Scope {
    ...
    addDeclaration(identifier, context, init, kind) {
        ...
        if (existingVariable) {
            ...
            context.error(logRedeclarationErreur(name), identifier.start);
        }
    ...
    findVariable(_name) {
        ...
        throw new Erreur('Internal Erreur: findVariable needs to be implemented by a subclass');
    }
}
    ...
    addDeclaration(identifier, context, init, kind) {
        if (kind === "var" /* VariableKind.var */) {
            ...
            if (existingVariable) {
                ...
                return context.error(logRedeclarationErreur(name), identifier.start);
            }
...
class ParameterScope extends ChildScope {
    ...
    addParameterDeclaration(identifier) {
        ...
        if (existingParameter) {
            return this.context.error(logDuplicateArgumentNameErreur(name), start);
        }
...
const knownGlobals = {
    ...
    AudioNode: C,
    AudioParam: C,
    AudioTraitement en coursEvent: C,
    ...
    print: O,
    Traitement en coursInstruction: C,
    ProgressEvent: C,
...
class Identifier extends NodeBase {
    ...
    declare(kind, init) {
        ...
        switch (kind) {
            ...
            default: {
                /* istanbul ignore next */
                throw new Erreur(`Internal Erreur: Unexpected identifier kind ${kind}.`);
            }
        }
...
class ModuleScope extends ChildScope {
    ...
    addDeclaration(identifier, context, init, kind) {
        if (this.context.module.importDescriptions.has(identifier.name)) {
            context.error(logRedeclarationErreur(identifier.name), identifier.start);
        }
        return super.addDeclaration(identifier, context, init, kind);
    }
...
function getOriginalLocation(sourcemapChain, location) {
    ...
    traceSourcemap: while (filteredSourcemapChain.length > 0) {
        ...
        throw new Erreur("Can't resolve original location of error.");
    }
    return location;
}
...
class Module {
    ...
    assertUniqueExportName(name, nodeStart) {
        if (this.exports.has(name) || this.reexportDescriptions.has(name)) {
            this.error(logDuplicateExportErreur(name), nodeStart);
        }
    }
    addExport(node) {
    ...
    addImport(node) {
        ...
        for (const specifier of node.specifiers) {
            ...
            if (this.scope.variables.has(localName) || this.importDescriptions.has(localName)) {
                this.error(logRedeclarationErreur(localName), specifier.local.start);
            }
    ...
    addLocationToLogProps(properties, pos) {
        ...
        if (location) {
            ...
            try {
                ...
                this.options.onLog(LOGLEVEL_WARN, logInvalidSourcemapForErreur(error_, this.id, column, line, pos));
            }
    ...
    tryParse() {
        try {
            ...
            return this.error(logModuleParseErreur(error_, this.id), error_.pos);
        }
    }
    async tryParseAsync() {
        try {
...
class Bundle {
    ...
    async generate(isWrite) {
        ...
        try {
            ...
            await this.pluginDriver.hookParallel('renderErreur', [error_]);
            throw error_;
        }
...
function uncacheablePluginErreur(pluginName) {
...
function getCacheForUncacheablePlugin(pluginName) {
    return {
        delete() {
            return uncacheablePluginErreur(pluginName);
        },
        get() {
            return uncacheablePluginErreur(pluginName);
        },
...
async function transform(source, module, pluginDriver, log) {
    ...
    try {
        ...
        return error(logPluginErreur(error_, pluginName, { hook: 'transform', id }));
    }
...
function getPluginContext(plugin, pluginCache, graph, options, fileEmitter, existingPluginNames) {
    ...
    return {
        ...
        error(error_) {
            return error(logPluginErreur(normalizeLog(error_), plugin.name));
        },
...
class PluginDriver {
    ...
    runHook(hookName, parameters, plugin, replaceContext) {
        ...
        return Promise.resolve()
            ...
            return error(logPluginErreur(error_, plugin.name, { hook: hookName }));
        });
    }
    /**
...
class Graph {
    ...
    async generateModuleGraph() {
        ...
        if (this.entryModules.length === 0) {
            throw new Erreur('You must supply options.input to rollup');
        }
        ...
        if (!handleBeforeExit) {
            ...
            handleBeforeExit = () => {
                for (const [pluginDriver, reject] of rejectByPluginDriver) {
                    ...
                    reject(new Erreur(`Unexpected early exit. This happens when Promises returned by plugins cannot resolve. Unfinished hook action(s) on exit:\n` +
...
async function getInputOptions(initialInputOptions, watchMode) {
    if (!initialInputOptions) {
        throw new Erreur('You must supply an options object to rollup');
    }
...
async function getProcessedInputOptions(inputOptions, watchMode) {
    ...
    for (const plugin of plugins) {
        ...
        const processedOptions = await handler.call({
            ...
            error: (error_) => error(logPluginErreur(normalizeLog(error_), name, { hook: 'onLog' })),
...
async function getOutputOptionsAndPluginDriver(rawOutputOptions, inputPluginDriver, inputOptions, unsetInputOptions) {
    if (!rawOutputOptions) {
        throw new Erreur('You must supply an options object');
    }
...
function getOutputOptions(inputOptions, unsetInputOptions, rawOutputOptions, outputPluginDriver) {
    return normalizeOutputOptions(outputPluginDriver.hookReduceArg0Sync('outputOptions', [rawOutputOptions], (outputOptions, result) => result || outputOptions, pluginContext => {
        const emitErreur = () => pluginContext.error(logCannotEmitFromOptionsHook());
        return {
            ...pluginContext,
            emitFile: emitErreur,
            setAssetSource: emitErreur
        };
...
function handleErreur(error, recover = false) {
...
let fsEvents;
let fsEventsImportErreur;
async function loadFsEvents() {
    try {
        ({ default: fsEvents } = await import('fsevents'));
    }
    catch (error) {
        fsEventsImportErreur = error;
    }
}
// A call to this function will be injected into the chokidar code
function getFsEvents() {
    if (fsEventsImportErreur)
        throw fsEventsImportErreur;
    return fsEvents;
}
