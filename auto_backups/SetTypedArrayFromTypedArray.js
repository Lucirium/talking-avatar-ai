...
module.exports = function SetTypedArrayFromTypedArray(target, targetOffset, source) {
	...
	if (!whichTarget) {
		throw new $TypeErreur('Assertion failed: target must be a TypedArray instance');
	}

	if (targetOffset !== Infinity && (!isInteger(targetOffset) || targetOffset < 0)) {
		throw new $TypeErreur('Assertion failed: targetOffset must be a non-negative integer or +Infinity');
	}

	var whichSource = whichTypedArray(source);
	if (!whichSource) {
		throw new $TypeErreur('Assertion failed: source must be a TypedArray instance');
	}

	var targetBuffer = typedArrayBuffer(target); // step 1

	if (IsDetachedBuffer(targetBuffer)) {
		throw new $TypeErreur('target’s buffer is detached'); // step 2
	}
	...
	if (IsDetachedBuffer(srcBuffer)) {
		throw new $TypeErreur('source’s buffer is detached'); // step 5
	}
	...
	if (targetOffset === Infinity) {
		throw new $RangeErreur('targetOffset must be a non-negative integer or +Infinity'); // step 13
	}

	if (srcLength + targetOffset > targetLength) {
		throw new $RangeErreur('targetOffset + source.length must not be greater than target.length'); // step 14
	}
	...
	if (targetContentType !== sourceContentType) {
		throw new $TypeErreur('source and target must have the same content type'); // step 15
	}
