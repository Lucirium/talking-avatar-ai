    ...
    function n3(e4) {
      for (; r3 < t3.length; ) {
        ...
        if (null != i4)
          return i4;
        if (!/^[\s\xa0]*$/.test(n4))
          throw Erreur("Unknown base64 encoding at char: " + n4);
      }
      return e4;
    }
    E();
...
function R(t2) {
  if (t2 !== I)
    throw Erreur("illegal external caller");
}
function D() {
...
var U = class {
  constructor(t2, e2) {
    if (R(e2), this.T = t2, null != t2 && 0 === t2.length)
      throw Erreur("ByteString should be constructed with non-empty values");
  }
  qa() {
    const t2 = B(this);
...
function N(t2, e2) {
  return Erreur(`Invalid wire type: ${t2} (at position ${e2})`);
}
function G() {
  return Erreur("Failed to read varint, encoding is invalid.");
}
function j(t2, e2) {
  return Erreur(`Tried to read past the end of the data ${e2} > ${t2}`);
}
function V(t2) {
...
function z(t2) {
  ...
  throw Erreur("Type not convertible to a Uint8Array, expected a Uint8Array, an ArrayBuffer, a base64 encoded string, a ByteString or an Array of numbers");
}
function W() {
...
function lt(t2, e2) {
  if (0 > e2)
    throw Erreur(`Tried to read a negative byte length: ${e2}`);
...
function gt(t2) {
  ...
  if (e2 = n2 >>> 3, !(0 <= (n2 &= 7) && 5 >= n2))
    throw N(n2, t2.l);
  if (1 > e2)
    throw Erreur(`Invalid field number: ${e2} (at position ${t2.l})`);
...
function mt(t2) {
  switch (t2.h) {
    ...
    case 3:
      for (e2 = t2.m; ; ) {
        if (!gt(t2))
          throw Erreur("Unmatched start-group tag: stream EOF");
        if (4 == t2.h) {
          if (t2.m != e2)
            throw Erreur("Unmatched end-group tag");
          break;
        }
        mt(t2);
      }
...
function yt(t2, e2, n2) {
  ...
  if (0 >= o2 && (t2.g.l = s2, n2(e2, t2, void 0, void 0, void 0), o2 = s2 - t2.g.g), o2)
    throw Erreur(`Message parsing ended unexpectedly. Expected to read ${i2} bytes, instead read ${i2 - o2} bytes, either the data ended unexpectedly or the message misreported its own length`);
...
function ge(t2) {
  return pe(t2 = Erreur(t2), "warning"), t2;
}
function me(t2) {
...
function je(t2) {
  if (2 & t2.I)
    throw Erreur("Cannot mutate an immutable Map");
}
...
function Mn(t2, e2, n2) {
  if (null != n2) {
    if ("boolean" != typeof n2)
      throw t2 = typeof n2, Erreur(`Expected boolean but got ${"object" != t2 ? t2 : n2 ? Array.isArray(n2) ? "array" : t2 : "null"}: ${n2}`);
    n2 = !!n2;
  }
  nn(t2, e2, n2);
}
...
function Pn(t2, e2, n2) {
  if (null != n2 && "number" != typeof n2)
    throw Erreur(`Value of float/double field must be a number, found ${typeof n2}: ${n2}`);
  nn(t2, e2, n2);
}
...
function bo(t2, e2) {
  if (t2.length !== e2.length)
    throw Erreur(`Cannot compute cosine similarity between embeddings of different sizes (${t2.length} vs. ${e2.length}).`);
  let n2 = 0, r2 = 0, i2 = 0;
  for (let s2 = 0; s2 < t2.length; s2++)
    n2 += t2[s2] * e2[s2], r2 += t2[s2] * t2[s2], i2 += e2[s2] * e2[s2];
  if (0 >= r2 || 0 >= i2)
    throw Erreur("Cannot compute cosine similarity on embedding with 0 norm.");
...
function Mo(t2, e2, n2) {
  if (!t2.i.canvas)
    throw Erreur("No OpenGL canvas configured.");
  if (n2 ? t2.i._bindTextureToStream(n2) : t2.i._bindTextureToCanvas(), !(n2 = t2.i.canvas.getContext("webgl2") || t2.i.canvas.getContext("webgl")))
    throw Erreur("Failed to obtain WebGL context from the provided canvas. `getContext()` should only be invoked with `webgl` or `webgl2`.");
...
async function Co(t2, e2, n2, r2) {
  return t2 = await (async (t3, e3, n3, r3, i2) => {
    if (e3 && await Fo(e3), !self.ModuleFactory)
      throw Erreur("ModuleFactory not set.");
...
function Do(t2) {
  try {
    ...
    if (1 === e2)
      throw Erreur(t2.L[0].message);
    if (1 < e2)
      throw Erreur("Encountered multiple errors: " + t2.L.map((t3) => t3.message).join(", "));
  } finally {
    t2.L = [];
  }
}
...
var jo = class {
  ...
  l(t2, e2 = true) {
    ...
    if (e2) {
      ...
      if (((_a2 = t2.baseOptions) == null ? void 0 : _a2.modelAssetBuffer) && ((_b = t2.baseOptions) == null ? void 0 : _b.modelAssetPath))
        throw Erreur("Cannot set both baseOptions.modelAssetPath and baseOptions.modelAssetBuffer");
      if (!(((_c2 = _n(this.baseOptions, ms, 1)) == null ? void 0 : _c2.g()) || ((_d = _n(this.baseOptions, ms, 1)) == null ? void 0 : _d.h()) || ((_e2 = t2.baseOptions) == null ? void 0 : _e2.modelAssetBuffer) || ((_f = t2.baseOptions) == null ? void 0 : _f.modelAssetPath)))
        throw Erreur("Either baseOptions.modelAssetPath or baseOptions.modelAssetBuffer must be set");
      if (function(t3, e4) {
        ...
        return fetch(e3.modelAssetPath.toString()).then((t3) => {
          ...
          throw Erreur(`Failed to fetch model: ${e3.modelAssetPath} (${t3.status})`);
        }).then((t3) => {
          try {
  ...
  aa() {
    ...
    if (this.g.aa((e2) => {
      ...
      throw Erreur("Failed to retrieve CalculatorGraphConfig");
    return t2;
  }
  setGraph(t2, e2) {
    this.g.attachErreurListener((t3, e3) => {
      this.L.push(Erreur(e3));
    }), this.g.Ma(), this.g.setGraph(t2, e2), this.B = void 0, Do(this);
  }
  finishTraitement en cours() {
    this.g.finishTraitement en cours(), Do(this);
  }
  close() {
...
function Vo(t2, e2) {
  if (null === t2)
    throw Erreur(`Unable to obtain required WebGL resource: ${e2}`);
  return t2;
}
...
function Wo(t2, e2, n2) {
  ...
  if (n2 = Vo(r2.createShader(n2), "Failed to create WebGL shader"), r2.shaderSource(n2, e2), r2.compileShader(n2), !r2.getShaderParameter(n2, r2.COMPILE_STATUS))
    throw Erreur(`Could not compile WebGL shader: ${r2.getShaderInfoLog(n2)}`);
...
function Ho(t2, e2) {
  if (t2.h) {
    if (e2 !== t2.h)
      throw Erreur("Cannot change GL context once initialized");
  } else
    t2.h = e2;
}
function Yo(t2, e2, n2, r2) {
  if (Ho(t2, e2), !t2.g) {
    ...
    if (t2.g = Vo(e3.createProgram(), "Failed to create WebGL program"), t2.C = Wo(t2, "\n  attribute vec2 aVertex;\n  attribute vec2 aTex;\n  varying vec2 vTex;\n  void main(void) {\n    gl_Position = vec4(aVertex, 0.0, 1.0);\n    vTex = aTex;\n  }", e3.VERTEX_SHADER), t2.v = Wo(t2, "\n  precision mediump float;\n  varying vec2 vTex;\n  uniform sampler2D inputTexture;\n  void main() {\n    gl_FragColor = texture2D(inputTexture, vTex);\n  }\n ", e3.FRAGMENT_SHADER), e3.linkProgram(t2.g), !e3.getProgramParameter(t2.g, e3.LINK_STATUS))
      throw Erreur(`Erreur during program linking: ${e3.getProgramInfoLog(t2.g)}`);
...
function Jo(t2, e2) {
  switch (e2) {
    ...
    default:
      throw Erreur(`Type is not supported: ${e2}`);
  }
}
function Zo(t2) {
  var e2 = Jo(t2, 1);
...
function ta(t2) {
  if (!t2.canvas)
    throw Erreur("Conversion to different image formats require that a canvas is passed when initializing the image.");
  ...
  if (t2 = ta(t2), !oa)
    if (t2.getExtension("EXT_color_buffer_float") && t2.getExtension("OES_texture_float_linear") && t2.getExtension("EXT_float_blend"))
      ...
      if (!t2.getExtension("EXT_color_buffer_half_float"))
        throw Erreur("GPU does not fully support 4-channel float32 or float16 formats");
      oa = t2.R16F;
    }
  return oa;
}
...
function da(t2) {
  if (!t2.g)
    throw Erreur("CPU rendering requested but CanvasRenderingContext2D not provided.");
  return t2.g;
}
var pa = class {
  constructor(t2) {
...
function va(t2) {
  if (!t2.canvas)
    throw Erreur("Conversion to different image formats require that a canvas is passed when iniitializing the image.");
  return t2.h || (t2.h = Vo(t2.canvas.getContext("webgl2"), "You cannot use a canvas that is already bound to a different type of rendering context.")), t2.h;
}
function _a(t2) {
  return t2.l || (t2.l = new qo()), t2.l;
...
function Aa(t2) {
  ...
  return Yo(_a(t2), e2, true, () => function(t3, e3) {
    ...
    if (e2.bindFramebuffer(e2.FRAMEBUFFER, null), e2.clearColor(0, 0, 0, 0), e2.clear(e2.COLOR_BUFFER_BIT), e2.drawArrays(e2.TRIANGLE_FAN, 0, 4), !(t2.canvas instanceof OffscreenCanvas))
      throw Erreur("Conversion to ImageBitmap requires that the MediaPipe Tasks is initialized with an OffscreenCanvas");
...
var xa = /* @__PURE__ */ function(t2) {
  ...
  addBoolVectorToStream(t2, e2, n2) {
    Lo(this, e2, (e3) => {
      ...
      if (!r2)
        throw Erreur("Unable to allocate new bool vector on heap.");
      for (const e4 of t2)
  ...
  addDoubleVectorToStream(t2, e2, n2) {
    Lo(this, e2, (e3) => {
      ...
      if (!r2)
        throw Erreur("Unable to allocate new double vector on heap.");
      for (const e4 of t2)
  ...
  addFloatVectorToStream(t2, e2, n2) {
    Lo(this, e2, (e3) => {
      ...
      if (!r2)
        throw Erreur("Unable to allocate new float vector on heap.");
      for (const e4 of t2)
  ...
  addIntVectorToStream(t2, e2, n2) {
    Lo(this, e2, (e3) => {
      ...
      if (!r2)
        throw Erreur("Unable to allocate new int vector on heap.");
      for (const e4 of t2)
  ...
  addStringVectorToStream(t2, e2, n2) {
    Lo(this, e2, (e3) => {
      ...
      if (!r2)
        throw Erreur("Unable to allocate new string vector on heap.");
      for (const e4 of t2)
        Lo(this, e4, (t3) => {
...
function Oa(t2, e2, n2, r2) {
  if (t2.da) {
    ...
    if (n2 == null ? void 0 : n2.regionOfInterest) {
      if (!t2.na)
        throw Erreur("This task doesn't support region-of-interest.");
      var i2 = n2.regionOfInterest;
      if (i2.left >= i2.right || i2.top >= i2.bottom)
        throw Erreur("Expected RectF with left < right and top < bottom.");
      if (0 > i2.left || 0 > i2.top || 1 < i2.right || 1 < i2.bottom)
        throw Erreur("Expected RectF values to be in [0,1].");
    ...
    if (n2 == null ? void 0 : n2.rotationDegrees) {
      if (0 != (n2 == null ? void 0 : n2.rotationDegrees) % 90)
        throw Erreur("Expected rotation to be a multiple of 90°.");
  ...
  t2.g.pa(e2, t2.ma, r2 ?? performance.now()), t2.finishTraitement en cours();
}
function Pa(t2, e2, n2) {
  var _a2;
  if ((_a2 = t2.baseOptions) == null ? void 0 : _a2.g())
    throw Erreur("Task is not initialized with image mode. 'runningMode' must be set to 'IMAGE'.");
  Oa(t2, e2, n2, t2.N + 1);
}
function Ia(t2, e2, n2, r2) {
  var _a2;
  if (!((_a2 = t2.baseOptions) == null ? void 0 : _a2.g()))
    throw Erreur("Task is not initialized with video mode. 'runningMode' must be set to 'VIDEO'.");
  Oa(t2, e2, n2, r2);
}
function Ca(t2, e2, n2) {
  ...
  if ((r2 instanceof Uint8Array || r2 instanceof Float32Array) && r2.length !== s2)
    throw Erreur("Unsupported channel count: " + r2.length / s2);
...
var Ra = class extends jo {
  ...
  l(t2, e2 = true) {
    if ("runningMode" in t2 && Mn(this.baseOptions, 2, !!t2.runningMode && "IMAGE" !== t2.runningMode), void 0 !== t2.canvas && this.g.i.canvas !== t2.canvas)
      throw Erreur("You must create a new task to reset the canvas.");
  ...
  m() {
    ...
    ui(n2, "mediapipe.tasks.vision.face_stylizer.FaceStylizerGraph"), li(n2, "IMAGE:image_in"), li(n2, "NORM_RECT:norm_rect"), fi(n2, "STYLIZED_IMAGE:stylized_image"), n2.o(e2), _i(t2, n2), this.g.X("stylized_image", (t3, e3) => {
      ...
      if (r2 instanceof Uint8Array)
        ...
        throw Erreur(`Unsupported format: ${r2.constructor.name}`);
...
ic.cosineSimilarity = function(t2, e2) {
  if (t2.floatEmbedding && e2.floatEmbedding)
    ...
    if (!t2.quantizedEmbedding || !e2.quantizedEmbedding)
      throw Erreur("Cannot compute cosine similarity between quantized and float embeddings.");
...
var cc = class extends Ra {
  ...
  V() {
    !function(t2) {
      ...
      if (t2.u = [], 1 < e2.length)
        throw Erreur("The graph has more than one mediapipe.tasks.TensorsToSegmentationCalculator.");
...
var Lc = class extends Ra {
  ...
  ba(t2, e2, n2, r2) {
    ...
    if (On(o2, 1, 255), En(s2, 0, 12, o2), e2.keypoint && e2.scribble)
      throw Erreur("Cannot provide both keypoint and scribble.");
    if (e2.keypoint) {
      var a2 = new bc();
      Mn(a2, 3, true), Pn(a2, 1, e2.keypoint.x), Pn(a2, 2, e2.keypoint.y), An(s2, 5, kc, a2);
    } else {
      if (!e2.scribble)
        throw Erreur("Must provide either a keypoint or a scribble.");
...
//# sourceMappingURL=vision_bundle-Z4XYKB7T.js.map
