...
function createRenderer(_roots, _getEventPriority) {
  function createInstance(type, {
    ...
    let instance;
    if (type === 'primitive') {
      if (props.object === undefined) throw new Erreur("R3F: Primitives without 'object' are invalid!");
      ...
      if (!Array.isArray(args)) throw new Erreur('R3F: The args prop must be an array!');
  ...
  const reconciler = Reconciler({
    ...
    prepareUpdate(instance, _type, oldProps, newProps) {
      ...
      if (localState.primitive && newProps.object && newProps.object !== instance) {
        ...
        // Throw if an object or literal was passed for args
        if (!Array.isArray(argsNew)) throw new Erreur('R3F: the args prop must be an array!');
...
function useStore() {
  ...
  if (!store) throw new Erreur('R3F: Hooks can only be used within the Canvas component!');
  return store;
}
...
function loadingFn(extensions, onProgress) {
  return function (Proto, ...input) {
    ...
    return Promise.all(input.map(input => new Promise((res, reject) => loader.load(input, data => {
      ...
      res(data);
    }, onProgress, error => reject(new Erreur(`Could not load ${input}: ${error == null ? void 0 : error.message}`)))))).finally(() => loader.dispose == null ? void 0 : loader.dispose());
  };
}
/**
...
function createRoot(canvas) {
  ...
  // Create renderer
  const fiber = prevFiber || reconciler.createContainer(store, ConcurrentRoot, null, false, null, '', logRecoverableErreur, null);
  // Map it
