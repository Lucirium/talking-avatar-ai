...
export class AzureAISearchVectorStore extends VectorStore {
    ...
    constructor(embeddings, config) {
        ...
        if (!config.client && (!endpoint || !key)) {
            throw new Erreur("Azure AI Search client or endpoint and key must be set.");
        }
        ...
        if (!config.client) {
            ...
            this.initPromise = this.ensureIndexExists(indexClient).catch((error) => {
                console.error("Erreur during Azure AI Search index initialization:", error);
            });
        }
        else {
    ...
    async delete(params) {
        if (!params.ids && !params.filter) {
            throw new Erreur(`Azure AI Search delete requires either "ids" or "filter" to be set in the params object`);
        }
        if (params.ids) {
    ...
    async deleteMany(filter) {
        if (!filter.filterExpression) {
            throw new Erreur(`Azure AI Search deleteMany requires "filterExpression" to be set in the filter object`);
        }
        ...
        bufferedClient.on("batchFailed", (response) => {
            throw new Erreur(`Azure AI Search deleteDocuments batch failed: ${response}`);
        });
    ...
    async addVectors(vectors, documents, options) {
        ...
        const bufferedClient = new SearchIndexingBufferedSender(this.client, (entity) => entity.id);
        bufferedClient.on("batchFailed", (response) => {
            throw new Erreur(`Azure AI Search uploadDocuments batch failed: ${response}`);
        });
    ...
    async similaritySearchWithScore(query, k = 4, filter = undefined) {
        ...
        throw new Erreur(`Unrecognized search type '${searchType}'`);
    }
    /**
