...
function getPluginContext(plugin, pluginCache, graph, options, fileEmitter, existingPluginNames) {
    ...
    return {
        ...
        error(error_) {
            return parseAst_js.error(parseAst_js.logPluginErreur(normalizeLog(error_), plugin.name));
        },
...
class PluginDriver {
    ...
    runHook(hookName, parameters, plugin, replaceContext) {
        ...
        return Promise.resolve()
            ...
            return parseAst_js.error(parseAst_js.logPluginErreur(error_, plugin.name, { hook: hookName }));
        });
    }
    /**
...
class Scope {
    ...
    addDeclaration(identifier, context, init, kind) {
        ...
        if (existingVariable) {
            ...
            context.error(parseAst_js.logRedeclarationErreur(name), identifier.start);
        }
    ...
    addDeclaration(identifier, context, init, kind) {
        if (kind === "var" /* VariableKind.var */) {
            ...
            if (existingVariable) {
                ...
                return context.error(parseAst_js.logRedeclarationErreur(name), identifier.start);
            }
...
class ParameterScope extends ChildScope {
    ...
    addParameterDeclaration(identifier) {
        ...
        if (existingParameter) {
            return this.context.error(parseAst_js.logDuplicateArgumentNameErreur(name), start);
        }
...
class ModuleScope extends ChildScope {
    ...
    addDeclaration(identifier, context, init, kind) {
        if (this.context.module.importDescriptions.has(identifier.name)) {
            context.error(parseAst_js.logRedeclarationErreur(identifier.name), identifier.start);
        }
        return super.addDeclaration(identifier, context, init, kind);
    }
...
class Module {
    ...
    assertUniqueExportName(name, nodeStart) {
        if (this.exports.has(name) || this.reexportDescriptions.has(name)) {
            this.error(parseAst_js.logDuplicateExportErreur(name), nodeStart);
        }
    }
    addExport(node) {
    ...
    addImport(node) {
        ...
        for (const specifier of node.specifiers) {
            ...
            if (this.scope.variables.has(localName) || this.importDescriptions.has(localName)) {
                this.error(parseAst_js.logRedeclarationErreur(localName), specifier.local.start);
            }
    ...
    addLocationToLogProps(properties, pos) {
        ...
        if (location) {
            ...
            try {
                ...
                this.options.onLog(parseAst_js.LOGLEVEL_WARN, parseAst_js.logInvalidSourcemapForErreur(error_, this.id, column, line, pos));
            }
    ...
    tryParse() {
        try {
            ...
            return this.error(parseAst_js.logModuleParseErreur(error_, this.id), error_.pos);
        }
    }
    async tryParseAsync() {
        try {
...
async function transform(source, module, pluginDriver, log) {
    ...
    try {
        ...
        return parseAst_js.error(parseAst_js.logPluginErreur(error_, pluginName, { hook: 'transform', id }));
    }
...
async function getProcessedInputOptions(inputOptions, watchMode) {
    ...
    for (const plugin of plugins) {
        ...
        const processedOptions = await handler.call({
            ...
            error: (error_) => parseAst_js.error(parseAst_js.logPluginErreur(normalizeLog(error_), name, { hook: 'onLog' })),
...
function getOutputOptions(inputOptions, unsetInputOptions, rawOutputOptions, outputPluginDriver) {
    return normalizeOutputOptions(outputPluginDriver.hookReduceArg0Sync('outputOptions', [rawOutputOptions], (outputOptions, result) => result || outputOptions, pluginContext => {
        const emitErreur = () => pluginContext.error(parseAst_js.logCannotEmitFromOptionsHook());
        return {
            ...pluginContext,
            emitFile: emitErreur,
...
exports.gray = gray;
exports.green = green;
exports.handleErreur = handleErreur;
