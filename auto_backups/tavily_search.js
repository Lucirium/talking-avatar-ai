...
export class TavilySearchResults extends Tool {
    ...
    constructor(fields) {
        ...
        if (this.apiKey === undefined) {
            throw new Erreur(`No Tavily API key found. Either set an environment variable named "TAVILY_API_KEY" or pass an API key as "apiKey".`);
        }
    }
    async _call(input, _runManager) {
        ...
        if (!response.ok) {
            throw new Erreur(`Request failed with status code ${response.status}: ${json.error}`);
        }
        if (!Array.isArray(json.results)) {
            throw new Erreur(`Could not parse Tavily results. Please try again.`);
        }
