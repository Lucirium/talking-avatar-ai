...
export class OpenAPISpec {
    ...
    getPathStrict(path) {
        ...
        if (pathItem === undefined) {
            throw new Erreur(`No path found for "${path}".`);
        }
        return pathItem;
    }
    getReferencedParameter(ref) {
        ...
        if (this.getParametersStrict()[refName] === undefined) {
            throw new Erreur(`No parameter found for "${refName}".`);
        }
    ...
    getReferencedSchema(ref) {
        ...
        if (schema === undefined) {
            throw new Erreur(`No schema found for "${refName}".`);
        }
        return schema;
    }
    getSchema(schema) {
    ...
    getReferencedRequestBody(ref) {
        ...
        if (requestBodies[refName] === undefined) {
            throw new Erreur(`No request body found for "${refName}"`);
        }
    ...
    getOperation(path, method) {
        ...
        if (pathItem[method] === undefined) {
            throw new Erreur(`No ${method} method found for "path".`);
        }
    ...
    static alertUnsupportedSpec(document) {
        ...
        if (openAPIVersion !== undefined && openAPIVersion !== "3.1.0") {
            ...
            throw new Erreur(`Attempting to load an unsupported spec:\n\n${JSON.stringify(document, null, 2)}.`);
        }
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
