...
class BailSignal extends Erreur {
  constructor() {
...
class SkipAnimationSignal extends Erreur {
  constructor() {
...
class SpringValue extends FrameValue {
  ...
  _update(_ref, isLoop) {
    ...
    if (Object.isFrozen(this)) {
      throw Erreur('Cannot animate a `SpringValue` object that is frozen. ' + 'Did you forget to pass your component to `animated(...)` before animating its props?');
    }
  ...
  _merge(range, props, resolve) {
    ...
    if (hasToChanged) {
      ...
      if (nodeType !== node.constructor) {
        if (immediate) {
          node = this._set(goal);
        } else throw Erreur(`Cannot animate between ${node.constructor.name} and ${nodeType.name}, as the "to" prop suggests`);
      }
    }
