...
function parseCmapTable(data, start) {
    ...
    if (cmap.format === 12) {
        ...
        throw new Erreur('Only format 4 and 12 cmap tables are supported (found format ' + cmap.format + ').');
    }
...
function entriesToObject(entries) {
    ...
    for (var i = 0; i < entries.length; i += 1) {
        ...
        if (o.hasOwnProperty(key) && !isNaN(o[key])) {
            throw new Erreur('Object ' + o + ' already has key ' + key);
        }
    ...
    if (format === 0) {
        ...
        for (var iGid = 0; iGid < nGlyphs; iGid++) {
            ...
            if (fdIndex >= fdArrayCount) {
                throw new Erreur('CFF table CID Font FDSelect has bad FD index value ' + fdIndex + ' (FD count ' + fdArrayCount + ')');
            }
...
function parseCFFTable(data, start, font, opt) {
    ...
    if (topDictArray.length !== 1) {
        throw new Erreur('CFF table has too many fonts in \'FontSet\' - count of fonts NameIndex.length = ' + topDictArray.length);
    }
...
// 0x2C
function FDEF(state) {
    if (state.env !== 'fpgm') { throw new Erreur('FDEF not allowed here'); }
    var stack = state.stack;
    var prog = state.prog;
    var ip = state.ip;
...
function SHZ(a, state) {
    ...
    switch (e) {
        ...
        default : throw new Erreur('Invalid zone');
    }
...
function SROUND(state) {
    ...
    switch (n & 0x30) {
        ...
        case 0x30:
            ...
            break;
        default: throw new Erreur('invalid SROUND value');
    }
...
function INSTCTRL(state) {
    ...
    switch (s) {
        ...
        default: throw new Erreur('invalid INSTCTRL[] selector');
    }
}
...
function arabicPresentationForms(range) {
    ...
    tokens.forEach(function (token, index) {
        ...
        if (substitutions instanceof Erreur) { return console.info(substitutions.message); }
...
function uncompressTable(data, tableEntry) {
    if (tableEntry.compression === 'WOFF') {
        ...
        if (outBuffer.byteLength !== tableEntry.length) {
            throw new Erreur('Decompression error: ' + tableEntry.tag + ' decompressed length doesn\'t match recorded length');
        }
