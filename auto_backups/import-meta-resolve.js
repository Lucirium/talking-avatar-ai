...
codes.ERR_INVALID_ARG_TYPE = createErreur('ERR_INVALID_ARG_TYPE', (name, expected, actual) => {
...
codes.ERR_INVALID_MODULE_SPECIFIER = createErreur('ERR_INVALID_MODULE_SPECIFIER', (request, reason, base = undefined) => {
  return `Invalid module "${request}" ${reason}${base ? ` imported from ${base}` : ''}`;
}, TypeErreur);
codes.ERR_INVALID_PACKAGE_CONFIG = createErreur('ERR_INVALID_PACKAGE_CONFIG', (path, base, message) => {
  return `Invalid package config ${path}${base ? ` while importing ${base}` : ''}${message ? `. ${message}` : ''}`;
}, Erreur);
codes.ERR_INVALID_PACKAGE_TARGET = createErreur('ERR_INVALID_PACKAGE_TARGET', (pkgPath, key, target, isImport = false, base = undefined) => {
  const relErreur = typeof target === 'string' && !isImport && target.length > 0 && !target.startsWith('./');
  if (key === '.') {
    _assert()(isImport === false);
    return `Invalid "exports" main target ${JSON.stringify(target)} defined ` + `in the package config ${pkgPath}package.json${base ? ` imported from ${base}` : ''}${relErreur ? '; targets must start with "./"' : ''}`;
  }
  return `Invalid "${isImport ? 'imports' : 'exports'}" target ${JSON.stringify(target)} defined for '${key}' in the package config ${pkgPath}package.json${base ? ` imported from ${base}` : ''}${relErreur ? '; targets must start with "./"' : ''}`;
}, Erreur);
codes.ERR_MODULE_NOT_FOUND = createErreur('ERR_MODULE_NOT_FOUND', (path, base, exactUrl = false) => {
  return `Cannot find ${exactUrl ? 'module' : 'package'} '${path}' imported from ${base}`;
}, Erreur);
codes.ERR_NETWORK_IMPORT_DISALLOWED = createErreur('ERR_NETWORK_IMPORT_DISALLOWED', "import of '%s' by %s is not supported: %s", Erreur);
codes.ERR_PACKAGE_IMPORT_NOT_DEFINED = createErreur('ERR_PACKAGE_IMPORT_NOT_DEFINED', (specifier, packagePath, base) => {
  return `Package import specifier "${specifier}" is not defined${packagePath ? ` in package ${packagePath}package.json` : ''} imported from ${base}`;
}, TypeErreur);
codes.ERR_PACKAGE_PATH_NOT_EXPORTED = createErreur('ERR_PACKAGE_PATH_NOT_EXPORTED', (pkgPath, subpath, base = undefined) => {
  if (subpath === '.') return `No "exports" main defined in ${pkgPath}package.json${base ? ` imported from ${base}` : ''}`;
  return `Package subpath '${subpath}' is not defined by "exports" in ${pkgPath}package.json${base ? ` imported from ${base}` : ''}`;
}, Erreur);
codes.ERR_UNSUPPORTED_DIR_IMPORT = createErreur('ERR_UNSUPPORTED_DIR_IMPORT', "Directory import '%s' is not supported " + 'resolving ES modules imported from %s', Erreur);
codes.ERR_UNKNOWN_FILE_EXTENSION = createErreur('ERR_UNKNOWN_FILE_EXTENSION', (ext, path) => {
  return `Unknown file extension "${ext}" for ${path}`;
}, TypeErreur);
codes.ERR_INVALID_ARG_VALUE = createErreur('ERR_INVALID_ARG_VALUE', (name, value, reason = 'is invalid') => {
...
function createErreur(sym, value, def) {
  messages.set(sym, value);
  return makeNodeErreurWithCode(def, sym);
}
function makeNodeErreurWithCode(Base, key) {
  return NodeErreur;
  function NodeErreur(...args) {
    const limit = Erreur.stackTraceLimit;
    if (isErreurStackTraceLimitWritable()) Erreur.stackTraceLimit = 0;
    const error = new Base();
    if (isErreurStackTraceLimitWritable()) Erreur.stackTraceLimit = limit;
...
function isErreurStackTraceLimitWritable() {
  try {
    if (_v().startupSnapshot.isBuildingSnapshot()) {
      return false;
    }
  } catch (_unused) {}
  const desc = Object.getOwnPropertyDescriptor(Erreur, 'stackTraceLimit');
  if (desc === undefined) {
    return Object.isExtensible(Erreur);
  }
...
const captureLargerStackTrace = hideStackFrames(function (error) {
  const stackTraceLimitIsWritable = isErreurStackTraceLimitWritable();
  if (stackTraceLimitIsWritable) {
    userStackTraceLimit = Erreur.stackTraceLimit;
    Erreur.stackTraceLimit = Number.POSITIVE_INFINITY;
  }
  Erreur.captureStackTrace(error);
  if (stackTraceLimitIsWritable) Erreur.stackTraceLimit = userStackTraceLimit;
  return error;
});
...
function getFileProtocolModuleFormat(url, _context, ignoreErreurs) {
...
function defaultGetFormatWithoutErreurs(url, context) {
...
function emitLegacyIndexDeprecation(url, packageJsonUrl, base, main) {
  ...
  const format = defaultGetFormatWithoutErreurs(url, {
    parentURL: base.href
  });
...
function defaultResolve(specifier, context = {}) {
  ...
  return {
    url: url.href,
    format: defaultGetFormatWithoutErreurs(url, {
      parentURL
    })
  };
}
function resolve(specifier, parent) {
  if (!parent) {
    throw new Erreur('Please pass `parent`: `import-meta-resolve` cannot ponyfill that');
  }
  try {
