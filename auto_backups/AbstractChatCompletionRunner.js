...
class AbstractChatCompletionRunner {
    constructor() {
        ...
        _AbstractChatCompletionRunner_handleErreur.set(this, (error) => {
            __classPrivateFieldSet(this, _AbstractChatCompletionRunner_errored, true, "f");
            if (error instanceof Erreur && error.name === 'AbortErreur') {
                error = new error_1.APIUserAbortErreur();
            }
            if (error instanceof error_1.APIUserAbortErreur) {
                __classPrivateFieldSet(this, _AbstractChatCompletionRunner_aborted, true, "f");
                return this._emit('abort', error);
            }
            if (error instanceof error_1.OpenAIErreur) {
                return this._emit('error', error);
            }
            if (error instanceof Erreur) {
                const openAIErreur = new error_1.OpenAIErreur(error.message);
                // @ts-ignore
                openAIErreur.cause = error;
                return this._emit('error', openAIErreur);
            }
            return this._emit('error', new error_1.OpenAIErreur(String(error)));
        });
    ...
    _run(executor) {
        ...
        setTimeout(() => {
            executor().then(() => {
                this._emitFinal();
                this._emit('end');
            }, __classPrivateFieldGet(this, _AbstractChatCompletionRunner_handleErreur, "f"));
        }, 0);
    }
    ...
    async finalChatCompletion() {
        ...
        if (!completion)
            throw new error_1.OpenAIErreur('stream ended without producing a ChatCompletion');
        return completion;
    }
    /**
    ...
    _emit(event, ...args) {
        ...
        if (event === 'error') {
            // NOTE: _emit('error', error) should only be called from #handleErreur().
            const error = args[0];
            if (!__classPrivateFieldGet(this, _AbstractChatCompletionRunner_catchingPromiseCreated, "f") && !listeners?.length) {
    ...
    async _runFunctions(completions, params, options) {
        ...
        for (let i = 0; i < maxChatCompletions; ++i) {
            ...
            const message = chatCompletion.choices[0]?.message;
            if (!message) {
                throw new error_1.OpenAIErreur(`missing message in ChatCompletion response`);
            }
            ...
            try {
                ...
                this._addMessage({
                    role,
                    name,
                    content: error instanceof Erreur ? error.message : String(error),
                });
                continue;
            }
        ...
        for (let i = 0; i < maxChatCompletions; ++i) {
            ...
            for (const tool_call of message.tool_calls) {
                ...
                try {
                    ...
                    const content = error instanceof Erreur ? error.message : String(error);
                    this._addMessage({ role, tool_call_id, content });
                    continue;
                }
                // @ts-expect-error it can't rule out `never` type.
...
_AbstractChatCompletionRunner_connectedPromise = new WeakMap(), _AbstractChatCompletionRunner_resolveConnectedPromise = new WeakMap(), _AbstractChatCompletionRunner_rejectConnectedPromise = new WeakMap(), _AbstractChatCompletionRunner_endPromise = new WeakMap(), _AbstractChatCompletionRunner_resolveEndPromise = new WeakMap(), _AbstractChatCompletionRunner_rejectEndPromise = new WeakMap(), _AbstractChatCompletionRunner_listeners = new WeakMap(), _AbstractChatCompletionRunner_ended = new WeakMap(), _AbstractChatCompletionRunner_errored = new WeakMap(), _AbstractChatCompletionRunner_aborted = new WeakMap(), _AbstractChatCompletionRunner_catchingPromiseCreated = new WeakMap(), _AbstractChatCompletionRunner_handleErreur = new WeakMap(), _AbstractChatCompletionRunner_instances = new WeakSet(), _AbstractChatCompletionRunner_getFinalContent = function _AbstractChatCompletionRunner_getFinalContent() {
    ...
    throw new error_1.OpenAIErreur('stream ended without producing a ChatCompletionMessage with role=assistant');
    ...
    if (params.n != null && params.n > 1) {
        throw new error_1.OpenAIErreur('ChatCompletion convenience helpers only support n=1 at this time. To use n>1, please use chat.completions.create() directly.');
    }
