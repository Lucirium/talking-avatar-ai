...
const _KTX2Loader = class extends Loader {
  ...
  load(url, onLoad, onProgress, onErreur) {
    if (this.workerConfig === null) {
      throw new Erreur("THREE.KTX2Loader: Missing initialization with `.detectSupport( renderer )`.");
    }
    ...
    loader.load(
      ...
      (buffer) => {
        if (_taskCache.has(buffer)) {
          ...
          return cachedTask.promise.then(onLoad).catch(onErreur);
        }
        this._createTexture(buffer).then((texture) => onLoad ? onLoad(texture) : null).catch(onErreur);
      },
      onProgress,
      onErreur
    );
...
__publicField(KTX2Loader, "BasisWorker", function() {
  ...
  function transcode(buffer) {
    ...
    if (!ktx2File.isValid()) {
      cleanup();
      throw new Erreur("THREE.KTX2Loader:	Invalid or unsupported .ktx2 file");
    }
    ...
    if (!width || !height || !levelCount) {
      cleanup();
      throw new Erreur("THREE.KTX2Loader:	Invalid texture");
    }
    if (!ktx2File.startTranscoding()) {
      cleanup();
      throw new Erreur("THREE.KTX2Loader: .startTranscoding failed");
    }
    const faces = [];
    const buffers = [];
    for (let face = 0; face < faceCount; face++) {
      ...
      for (let mip = 0; mip < levelCount; mip++) {
        ...
        for (let layer = 0; layer < layerCount; layer++) {
          ...
          if (!status) {
            cleanup();
            throw new Erreur("THREE.KTX2Loader: .transcodeImage failed.");
          }
          layerMips.push(dst);
        }
...
async function createRawTexture(container) {
  ...
  if (FORMAT_MAP[vkFormat] === void 0) {
    throw new Erreur("THREE.KTX2Loader: Unsupported vkFormat.");
  }
  let zstd;
  ...
  for (let levelIndex = 0; levelIndex < container.levels.length; levelIndex++) {
    ...
    if (container.supercompressionScheme === KHR_SUPERCOMPRESSION_NONE) {
      ...
      throw new Erreur("THREE.KTX2Loader: Unsupported supercompressionScheme.");
    }
    let data;
  ...
  if (UNCOMPRESSED_FORMATS.has(FORMAT_MAP[vkFormat])) {
    ...
    if (container.pixelDepth > 0)
      throw new Erreur("THREE.KTX2Loader: Unsupported pixelDepth.");
