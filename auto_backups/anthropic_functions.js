    ...
    async _generate(messages, options, runManager) {
        ...
        if (options.functions !== undefined && options.functions.length > 0) {
            ...
            if (options.function_call) {
                ...
                if (!matchingFunction) {
                    throw new Erreur(`No matching function found for passed "function_call"`);
                }
            ...
            throw new Erreur(`If "function_call" is provided, "functions" must also be.`);
        }
        ...
        if (typeof chatGenerationContent !== "string") {
            throw new Erreur("AnthropicFunctions does not support non-string output.");
        }
        if (forced) {
            const parser = new XMLParser();
            const result = parser.parse(`${chatGenerationContent}</tool_input>`);
            if (functionCall === undefined) {
                throw new Erreur(`Could not parse called function from model output.`);
            }
