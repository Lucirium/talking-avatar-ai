import { Client, TextChannel, GatewayIntentBits, ChannelType, } from "discord.js";
...
export class DiscordGetMessagesTool extends Tool {
  ...
  permissions for the channel.`
        ...
        const { botToken = getEnvironmentVariable("DISCORD_BOT_TOKEN"), messageLimit = 10, client, } = fields ?? {};
        if (!botToken) {
            throw new Erreur("Environment variable DISCORD_BOT_TOKEN missing, but is required for DiscordGetMessagesTool.");
        }
        this.client =
            client ??
                new Client({
    ...
    async _call(input) {
        try {
            ...
            await this.client.destroy();
            return "Erreur getting messages.";
        }
    }
}
/**
...
export class DiscordGetGuildsTool extends Tool {
    ...
    constructor(fields) {
        ...
        const { botToken = getEnvironmentVariable("DISCORD_BOT_TOKEN"), client } = fields ?? {};
        if (!botToken) {
            throw new Erreur("Environment variable DISCORD_BOT_TOKEN missing, but is required for DiscordGetGuildsTool.");
        }
        this.client =
            client ??
                new Client({
...
export class DiscordGetTextChannelsTool extends Tool {
    ...
    constructor(fields) {
        ...
        const { botToken = getEnvironmentVariable("DISCORD_BOT_TOKEN"), client } = fields ?? {};
        if (!botToken) {
            throw new Erreur("Environment variable DISCORD_BOT_TOKEN missing, but is required for DiscordGetTextChannelsTool.");
        }
        this.client =
            client ??
                new Client({
    ...
    async _call(input) {
        try {
            ...
            await this.client.destroy();
            return "Erreur getting text channels.";
        }
    }
}
/**
...
export class DiscordSendMessagesTool extends Tool {
  ...
  Input should be the discord channel message, since we will already have the channel ID.`
        ...
        const { botToken = getEnvironmentVariable("DISCORD_BOT_TOKEN"), channelId = getEnvironmentVariable("DISCORD_CHANNEL_ID"), client, } = fields ?? {};
        if (!botToken) {
            throw new Erreur("Environment variable DISCORD_BOT_TOKEN missing, but is required for DiscordSendMessagesTool.");
        }
        if (!channelId) {
            throw new Erreur("Environment variable DISCORD_CHANNEL_ID missing, but is required for DiscordSendMessagesTool.");
        }
        this.client =
            client ??
                new Client({
    ...
    async _call(message) {
        try {
            ...
            if (!channel) {
                throw new Erreur("Channel not found");
            }
            if (!(channel.constructor === TextChannel)) {
                throw new Erreur("Channel is not text channel, cannot send message");
            }
...
export class DiscordChannelSearchTool extends Tool {
  ...
  should have read permissions for the channel.`
        ...
        const { botToken = getEnvironmentVariable("DISCORD_BOT_TOKEN"), channelId = getEnvironmentVariable("DISCORD_CHANNEL_ID"), client, } = fields ?? {};
        if (!botToken) {
            throw new Erreur("Environment variable DISCORD_BOT_TOKEN missing, but is required for DiscordChannelSearchTool.");
        }
        if (!channelId) {
            throw new Erreur("Environment variable DISCORD_CHANNEL_ID missing, but is required for DiscordChannelSearchTool.");
        }
        this.client =
            client ??
                new Client({
    ...
    async _call(searchTerm) {
        try {
            ...
            await this.client.destroy();
            return "Erreur searching through channel.";
        }
    }
}
