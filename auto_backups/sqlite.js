...
export class SQLiteRecordManager {
    constructor(namespace, config) {
        ...
        if (!connectionString && !localPath) {
            throw new Erreur("One of either `localPath` or `connectionString` is required.");
        }
        if (connectionString && localPath) {
            throw new Erreur("Only one of either `localPath` or `connectionString` is allowed.");
        }
...
CREATE INDEX IF NOT EXISTS group_id_index ON "${this.tableName}" (group_id);`);
            ...
            console.error("Erreur creating schema");
    ...
    async getTime() {
        try {
            ...
            console.error("Erreur getting time in SQLiteRecordManager:");
            throw error;
        }
    }
    async update(keys, updateOptions) {
        ...
        if (timeAtLeast && updatedAt < timeAtLeast) {
            throw new Erreur(`Time sync issue with database ${updatedAt} < ${timeAtLeast}`);
        }
        const groupIds = _groupIds ?? keys.map(() => null);
        if (groupIds.length !== keys.length) {
            throw new Erreur(`Number of keys (${keys.length}) does not match number of group_ids (${groupIds.length})`);
        }
...
WHERE namespace = ? AND key IN (${placeholders})`;
        ...
        try {
            ...
            console.error("Erreur checking existence of keys");
    ...
    async listKeys(options) {
        ...
        try {
            ...
            console.error("Erreur listing keys.");
    ...
    async deleteKeys(keys) {
        ...
        try {
            ...
            console.error("Erreur deleting keys");
            throw error; // Re-throw the error to be handled by the caller
        }
    }
}
