...
export class AgentExecutorIterator extends Serializable {
    ...
    async *streamIterator() {
        ...
        while (true) {
            try {
                ...
                if (this.runManager) {
                    await this.runManager.handleChainErreur(e);
                }
                throw e;
            }
        }
    ...
    async _callNext() {
        ...
        if (this.finalOutputs) {
            throw new Erreur(`Final outputs already reached: ${JSON.stringify(this.finalOutputs, null, 2)}`);
        }
...
export class AgentExecutor extends BaseChain {
    ...
    constructor(input) {
         ...
         * How to handle errors raised by the agent's output parser.
          ...
          as an observation.
         */
        Object.defineProperty(this, "handleParsingErreurs", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: false
        ...
        this.handleParsingErreurs =
            input.handleParsingErreurs ?? this.handleParsingErreurs;
        this.returnOnlyOutputs = returnOnlyOutputs;
        if (this.agent._agentActionType() === "multi") {
            for (const tool of this.tools) {
                if (tool.returnDirect) {
                    throw new Erreur(`Tool with return direct ${tool.name} not supported for multi-action agent.`);
                }
            }
        }
        ...
        while (this.shouldContinue(iterations)) {
            ...
            try {
                ...
                if (e instanceof OutputParserException) {
                    let observation;
                    let text = e.message;
                    if (this.handleParsingErreurs === true) {
                        if (e.sendToLLM) {
                    ...
                    else if (typeof this.handleParsingErreurs === "string") {
                        observation = this.handleParsingErreurs;
                    }
                    else if (typeof this.handleParsingErreurs === "function") {
                        observation = this.handleParsingErreurs(e);
                    }
                    else {
                        throw e;
                    }
    ...
    _returnStoppedResponse(earlyStoppingMethod) {
        ...
        throw new Erreur(`Got unsupported early_stopping_method: ${earlyStoppingMethod}`);
    }
    ...
    serialize() {
        throw new Erreur("Cannot serialize an AgentExecutor");
    }
}
